...
interface View { last: Snap | null, plan: { code: string, name: string } | null, limits: { users: number | null, storageGb: number | null, smsPerMonth: number | null }, history: { collectedAt: string, activeUsers: number, storageBytes: number }[] }
const view = ref<View | null>(null)
const error = ref('')
onMounted(async () => { try { view.value = await api<View>('/settings/usage') } catch (err) { error.value = apiErrorOf(err).message } })

const gb = (b: number) => (b / 1024 ** 3).toLocaleString('uk-UA', { maximumFractionDigits: 1 })
