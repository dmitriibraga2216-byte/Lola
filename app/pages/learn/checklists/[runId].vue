    ...
    <template v-if="run">
      ...
      <section class="group">
        <div v-for="it in run.checklist.items" :key="it.id" class="item">
          ...
          <div v-if="ans(it.id)?.photoMediaIds?.length" class="photos"><img v-for="m in ans(it.id)!.photoMediaIds" :key="m" :src="`/api/v1/media/${m}`" alt=""></div>
        </div>
      </section>
