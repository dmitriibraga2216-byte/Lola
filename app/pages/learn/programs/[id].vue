    ...
    <template v-if="l">
      ...
      <ol class="ladder">
        <li v-for="(s, i) in l.steps" :key="s.id" :class="['step', s.state.status]" :data-testid="`step-${s.id}`">
          <span class="num">{{ s.state.status === 'done' ? '✓' : s.state.status === 'locked' ? '🔒' : s.state.status === 'failed' ? '✕' : i + 1 }}</span>
          <div class="body">
            <b>{{ s.title }}</b>
...
.ladder { list-style: none; margin: var(--space-3) 0 0; padding: 0; display: grid; gap: var(--space-2); }
.step { display: flex; gap: var(--space-3); background: var(--color-bg-soft); border-radius: var(--radius-l); padding: var(--space-3); }
.num { width: 36px; height: 36px; border-radius: 18px; display: grid; place-items: center; font-weight: 900; background: var(--color-bg-line); color: var(--color-ink-muted); flex: none; }
