...
- `[дополнено, fix-bot-login]` `GET /tg/go?t=<токен>&to=<путь>&n=<уведомление>` — переход по
  кнопке. Токен выпускается при отправке сообщения (`docs/23` §6 п. 7); вход — только по нему,
  `to` — путь этого же сайта (иначе главная). Отказ — редирект `/login?error=tg_link_expired`,
  один на все причины; кто уже вошёл в этом браузере, идёт сразу на `to`. `hitRateLimit` —
  60 переходов с адреса за 10 минут, дальше `/login?error=tg_too_many`. `/menu` присылает
  свежую кнопку.
