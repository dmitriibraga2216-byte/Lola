...
10. **AI-генерация текста.** Вход модели: название вакансии, должность, категория, город, тип занятости, формат,
    ...
    > [дополнено, PR-27] Модель — через шлюз `server/services/ai/gateway.ts` (`docs/v2/30` §3.2):
    > профиль роли `generate`, строка `ai_calls` на вызов (`ref_kind = 'vacancy_generation'`,
    > `ref_id` — строка `vacancy_ai_generations`), списание, текст блока и строка журнала генераций —
    > одной транзакцией. Истёкшая или выключенная ИИ-подписка — `409 ai.unavailable`
    > (`vacancy_ai_generations.status = 'limited'`), провайдер не ответил ни основным профилем, ни
    > запасным — `503 ai.provider_failed` (`status = 'failed'`); ни то ни другое не списывает.
