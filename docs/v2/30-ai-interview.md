...
[решение] **Собеседование — режим теста, а не вторая система прохождения.** Сценарий привязан к `quizzes` (`kind='interview'`), сессия — строка `attempts`, каждая реплика кандидата — строка `attempt_answers`. Всё, что работает для теста, работает и здесь: `snapshot` фиксирует вопросы на момент старта, `params` — правила из назначения (инвариант 1: попытки, дедлайн и проходной балл живут в назначении, а не в сценарии), статусы попытки из `12` §4 не переопределяются, `attempt_no` считается так же. [решение] **Голос — не новый тип вопроса, а способ ввода ответа**: вопросы собеседования имеют тип `text_long` (`12` §3.3), новое поле `attempt_answers.input_mode` говорит, чем ответили. Ручная проверка, статистика и очередь `13` §5.2 работают без изменений. Новые таблицы держат **только то, чего нет в модели попытки**: согласие, характеристики записи, качество расшифровки, обрывы связи, флаги антифрода; баллы и статусы в них не дублируются.

> [исправлено, PR-28 по решению `44` В-12] Ранее: «Сценарий привязан к `quizzes` (`mode='interview'`)».
> Колонки `mode` нет: вид теста — третье значение `quizzes.kind` (`quiz | certification | interview`,
> CHECK `quizzes_kind_chk`, миграция `0095_v2_interview`). Обычный путь теста для такого теста закрыт
> на сервере (`409 interview_consent.required`), а сменить вид в/из `interview` нельзя, пока по тесту
> есть попытки (`409 quiz.kind_locked`). Ответ голосом — `attempt_answers.input_mode = 'voice'`, его
> текст (расшифровка) кладётся в `attempt_answers.answer.text`: ручная проверка читает его там же, где
> письменный ответ. Попытку ИИ-сессии завершает сессия, прямые «сохранить ответ» и «отправить» к ней
> не применяются; сутками простоя она не сгорает — действует только дедлайн (`attempt.expire`).
...
> [исправлено, PR-28 — миграция `0095_v2_interview`] Ранее в DDL: «`alternative_path text not null
> default 'human_interview'`». Колонка без значения по умолчанию и пустая только у черновика: иначе
> критерий §13 к. 3 («сценарий без `alternative_path` не публикуется») был бы невыполним. Публикацию без
> альтернативы запрещает и сервис (`422 scenario.alternative_required`), и CHECK
> `interview_scenarios_alt_published_chk`. Сверх DDL: `published_at`; опубликованный сценарий — один на
> тест (частичный уникальный индекс), правка опубликованного создаёт версию-черновик с копией критериев;
> CHECK'и формы §6.1 (время, перезаписи, порог, язык); `record_video` — только `false` (видео — не
> PR-28); у критерия — CHECK длины названия 3–100. У `interview_consents` — `request_context` (журнал,
> CLAUDE.md п. 14), отказ без альтернативы и отзыв без `withdrawn_at` запрещены CHECK'ами; согласие
> принимается о паре «человек — версия сценария», сессии до согласия нет (к. 1, к. 2).
...
> [дополнено, PR-28] Сверх DDL: `interview_sessions.ai_stub` — явный признак, что хоть один вывод модели
> в сессии дал профиль-заглушка (`driver = 'stub'`), и `redacted_at` — записи стёрты отзывом согласия или
> обезличиванием (§7.6, §7.9); `degraded_reason` + `unexplained` — «оценка без обоснования» из таблицы
> §4 своего кода не имела. Реплика — одна строка на вопрос снимка (`role = 'candidate'`, вопрос
> интервьюера — `prompt_text`), все реплики заводятся на старте; текущая — первая без ответа. У
> письменного ответа «расшифровка» — сам текст (`transcript_status = 'not_needed'`). Индекс
> `idx_interview_turns_tenant` не заведён: он повторяет уникальный `(tenant_id, session_id, ordinal)`.
...
> [исправлено, PR-28] Ранее в DDL: «`rationale text not null`», «`check (length(rationale) between 20 and
> 2000)`», «`check (jsonb_array_length(evidence) >= 1)`». Обезличивание §7.9 требует стереть обоснование
> и цитаты, оставив балл, — а CHECK'и выше этого не допускали. Реализовано: `rationale` пуста **только**
> у стёртой строки (`redacted_at`), у любой живой — обоснование 20–2000 знаков, минимум одна цитата и у
> каждой цитаты непустой `quote` (`ics_evidence_quote_chk`); стёртая строка — без обоснования и без
> цитат (`ics_redacted_chk`); вставить уже «стёртую» строку нельзя (триггер `ics_insert_guard`) — так
> исключение не становится обходом. Индекс `idx_interview_criterion_scores_tenant` не заведён — он
> повторяет ведущие колонки уникального `(tenant_id, session_id, criterion_id)`.

> [исправлено, PR-29 — миграция `0096_v2_ai_summary`] `candidate_summaries` по DDL выше, с правками:
> - **строка «Документ сформовано автоматично» — CHECK таблицы** `candidate_summaries_disclaimer_chk`: в
>   `body.disclaimer.text` — начало подписи на языке документа (`shared/domain/candidateSummary.ts`,
>   `SUMMARY_DISCLAIMER`), иначе строка не сохраняется ни сервисом, ни правкой, ни прямой записью (§13 к. 14);
>   исключение — только стёртая строка (`redacted_at`, тело `{}`), и отправленной она быть не может;
> - сверх DDL: `auto_send_cancelled_at` / `auto_send_cancelled_by` — отмена авто-отправки до срока (§13 к. 13:
>   «может быть отменён»; иначе сверка поставила бы отменённое заново), `redacted_at` (§7.6, §7.9),
>   `updated_at`; CHECK'и перечней, `sent_chk` (у отправленного — токен, срок и момент), `revoked_chk`;
> - `sent_channel` получил перечень `email | link` («Скопіювати посилання» §5.4 — тоже отправка);
...
> [дополнено, PR-29 — миграция `0096_v2_ai_summary`] `ai_review_hints` — по DDL, без полей вердикта; сверх
> DDL `ai_stub` (подсказку дала заглушка, Р-28.4), `reason` (почему подсказки нет — §5.5 «причина видна
> админу»), `updated_at`; решение ментора и `agreement` пишутся только вместе (`ai_review_hints_decided_chk`).
> `key_source` — ключ на момент сверки: `{questionId, questionVersion | workshopId, criteriaIds, keyPoints:
> [{id, text}]}`. `ai_quality_reviews`: `sampled_by` получил перечень `auto | override` (`override` — несогласие
> рекрутера по форме §6.4 ставит строку сразу, §13 к. 6), `sample_reason` — `random | major |
> low_confidence | override_major | override_flagged`; вердикт и `reviewed_at` пишутся только вместе.
...
> [исправлено, PR-28 по решению `44` В-12] `alter table quizzes add column mode …` вычеркнут: вместо
> колонки — третье значение `quizzes.kind` и CHECK `quizzes_kind_chk` (миграция `0095_v2_interview`).
> `attempt_answers.input_mode` заведён раньше, миграцией `0078_v2_learning_time` (PR-21). Сверх §3.7:
> `candidate_scores.ai_stub` (Р-28.4) с CHECK «только у `kind = 'ai'`».
...
> [дополнено, PR-28] Как реализовано: строка сессии создаётся **в момент старта**, вместе с попыткой —
> `attempt_id` обязателен, а до решения по согласию попытки нет (к. 1); отказ сессию не создаёт вовсе
> (к. 2), поэтому `created` и `consent_pending` в продукте не встречаются — экран согласия адресуется
> тестом (`/interview/:quizId`), а не сессией. Обрыв: пауза при уходе со страницы или молчание канала
> дольше 30 с, замеченное при возврате, — `disconnects + 1`; `abandoned` по `interview.reap` без отзыва
> согласия — возобновляемое (попытка жива, §7.12 «посилання діє до {дата}»). Все реплики промолчаны —
> `failed` (`needs_human_reason = 'no_answers'`), попытка аннулируется, резерв ИИ снимается, рекрутеру —
> `interview_needs_human`. Дедлайн попытки истёк — `expired` с `degraded_reason = 'timeout'`.
...
> [дополнено, PR-29] Как реализовано. **Підсумок:** `draft` — пока собирается генеративная секция (данные
> секций — одной транзакцией, вызов модели — вне её), затем `ready`; модель недоступна или ответила со
> словами решения — секция пустая с пометкой, документ всё равно `ready`. Новая версия отзывает прежнюю
> отправленную (`revoked`, причина `superseded`) и снимает её плановую отправку — «старые версии недоступны
> по ссылке». Отзыв согласия и обезличивание стирают тело (`redacted_at`) и отзывают ссылку в транзакции
> стирания (`interview/redaction.ts`). **Подсказка:** `skipped` — сверять не с чем (нет ключа) или ответ пуст;
> `degraded` — ось `ai_review_ops` исчерпана или ИИ не действует; решение ментора по подсказке не в `ready`
> `agreement` не получает.
...
| Трек і модуль | селект | ✓ | — | тест с `kind='interview'` (`44` В-12) | «Оберіть модуль співбесіди» |
...
> [дополнено, PR-29] Форма §6.4 — `POST /candidates/:id/interview/criteria/:criterionId/override`
> `{humanValue, humanComment, major, expectedHumanAt}`: `expectedHumanAt` — момент прежнего несогласия,
> который видел рекрутер (оптимистическая блокировка §12 п. 6). Настройки §6.5 — группа
> `settings.recruiting.summaryAutoSend` (`enabled` по умолчанию **выключен**: документ кандидату без решения
> тенанта не уходит; при включении «Мінімальний бал» обязателен), экран «Налаштування → Рекрутинг». Ключа,
> который выключал бы строку «Документ сформовано автоматично», нет: неизвестный ключ правка настроек
> отклоняет `422`, а записанный мимо API — не читается схемой настроек.
...
> [дополнено, PR-28] Как реализованы правила §7 (решения — `46-progress.md`, «Фаза 3, PR-28»):
> - **§7.1–§7.2.** Вывод модели попадает в систему тремя путями, ни один не решает о человеке: текст
>   расшифровки — в реплику и ответ попытки; объяснённые баллы — в `interview_criterion_scores`; одно число
>   — в `candidate_scores.kind = 'ai'` через владельца оценок (`candidates.ts#writeAiScoreTx`). Модуль
>   собеседования не называет колонок решения о человеке — сквозная проверка 16 скрипта. Ответ модели
>   проверяется до записи (`validateScores`): у каждого критерия балл в шкале, уверенность, обоснование
>   20–2000 знаков и цитата, **дословно** найденная в ответе на названную реплику; иначе — один повтор с
>   усиленной инструкцией, затем `needs_human` (`unexplained`). **Заглушка (г):** профиль `stub` помечает
>   сессию и оценку явным признаком `ai_stub` (не по имени модели); карточка, вкладка «Співбесіда» и
>   `interview_completed` показывают «тестова заглушка, а не модель — не підстава для рішення». Заглушка
>   в боевом окружении не запрещена (демо-стенд и тенант без вендора работают сквозным путём), но без
>   пометки её число не показывается нигде (Р-28.4).
> - **§7.4–§7.5.** Текст согласия — версия юридического текста (`shared/domain/interview.ts`, три языка),
>   сервер отдаёт его экрану и хеширует ровно отданное; другая редакция — `422 interview_consent.invalid`.
>   Отказ окончателен для теста (`409 interview_consent.already_decided`); `human_interview` двигает
>   карточку в «На перевірці» (`candidateJobs.ts#interviewAlternativeTx`), `text_form` — открывает ту же
>   попытку письменной формой без ИИ. Альтернатива без отказа — `POST …/alternative` при недоступном ИИ
>   или без микрофона у сценария без текста; строки согласия она не пишет. Нейтральная строка в истории
>   — из `interview_consents` (`declined`, `withdrawn`), журнал колонок «переходов в себя» не получает.
> - **§7.6.** Отзыв — одна транзакция: согласие `withdrawn`, сессия `abandoned` (`consent_withdrawn`),
>   аудио — в корзину с `purge_after = now()`, расшифровки, вопросы, обоснования и цитаты, ответы попытки,
>   вход и выход вызовов модели стёрты, попытка **аннулирована** (не проваленная и не израсходованная).
>   Сессия без ответов возвращает резерв `ai_interview_ops` (`releaseSessionOp`, хвост PR-27).
> - **§7.7.** Полный вход оценки — файлом в S3 (`media_assets.origin = 'ai_artifact'`, владелец — кандидат)
>   и ключом в `ai_calls.input_ref`; у расшифровки вход — само аудио реплики (ссылкой, без копии).
>   `ai.calls_cleanup` через 90 дней обнуляет ссылку и отправляет файл входа в корзину. Аудио ответа —
>   `origin = 'interview_answer'`, до 25 МБ, `audio/webm|ogg|mp4|mpeg`; `interview_sessions.purge_after` —
>   `finished_at + 90 дней`, но не позже `users.consent_expires_at` (стирание — `interview.media_purge`, PR-29).
> - **§7.9.** Обезличивание — механикой `candidate.consent_sweep`: `anonymizeCandidate()` в той же
>   транзакции зовёт `interview/redaction.ts#redactInterviewData` (тот же код, что у отзыва), плюс стирает
>   IP, браузер и технический контекст согласий. Второго механизма сроков ПД нет.
> - **§7.10–§7.12.** Расшифровка по HTTP — multipart с аудио из S3 (`openai_compatible`/`self_hosted` —
>   `/audio/transcriptions`, `verbose_json`; `http_custom` — файл и поле `meta`); уверенность ниже порога
>   сценария — `low_confidence`, из доказательств исключается. Повторы расшифровки (5/30 мин) и оценки
>   (1/5/30 мин) ставит сам сервис. Биение раз в 5 с, `pause` на `pagehide` (`fetch` с `keepalive`),
>   черновик письменного ответа — на устройстве; каждая запись голоса вантажится сразу, поэтому
>   перезапись (§6.1) считает сервер (`409 retake.limit`), отброшенная запись уходит в корзину.

> [дополнено, PR-29] Как реализованы правила §7 (решения — `46-progress.md`, «Фаза 3, PR-29»):
> - **§7.3.** Балл человека — в `human_value` той же строки, расхождение считается сразу (`criterionAgreement`,
>   в долях шкалы). В карточку — **новая** `candidate_scores.kind = 'manual'` авторства человека со значением
>   итога сессии, пересчитанного с его баллом (остальные критерии — модели, свёртка §7.11); строка `kind = 'ai'`
>   не трогается. `major` или отметка «груба помилка моделі» ставят вывод модели в `ai_quality_reviews`
>   той же транзакцией; отметка кладёт и вердикт `major_error` авторства рекрутера (§6.4).
> - **§7.7.** Голос удаляется **физически** — и только он: `interview.media_purge` (03:40) сначала
>   назначает срок записям без срока (от конца сессии, у незавершённой — от последней активности, не позже
>   согласия на ПД), затем удаляет объекты S3 с истёкшим `purge_after` по одному (`DeleteObject`) и только
>   после подтверждения S3 переводит строку в `purged`. Отказ S3 не глушится: строка остаётся, в журнал —
>   `interview.media.purge_failed`, задача падает и повторяется. Корзина хранилища (`storage.purge`, объекты
>   не удаляет до решения владельца продукта, `44` §8) голос пропускает и из корзины его не восстанавливает
>   (`409 storage.not_restorable`): отзыв согласия и срок голоса — обещание кандидату. Тенанты обходятся
>   **все**, включая приостановленные. Срок ставится и на сами записи при завершении сессии.
> - **§7.13.** Триггер — постановка работы в очередь (`enqueueReview`, одна точка на все пути сдачи), при
>   включённой функции тенанта (`settings.ai.reviewHints`, по умолчанию **выключена**: включая её, тенант
>   решает отправлять ответы людей модели) — `ai_review_hints` `queued`, вызов модели — задача
>   `ai.review_hint`. Пункты ключа — критерии вопроса/практикума (иначе предложения эталона), текст пишет
>   методист, модель только сопоставляет; цитата — дословно из ответа, пояснение противоречия — без слов
>   вердикта (иначе повтор, затем `failed`). «Текст модуля» в PR-29 не передаётся. К ответам собеседования
>   (`quizzes.kind = 'interview'`) подсказки нет: у голосовой сессии своя оценка ИИ, письменную форму кандидат
>   выбрал, отказавшись от ИИ. Первое раскрытие панели ставит `shown_at`; `agreement` пишут
>   `attempts.ts#gradeManual` и `workshops.ts#grade` в транзакции решения. Форма ментора подсказку не читает.
> - **§7.14.** Секции — по документу; «оценки с авторами» у рекрутера, а в документе для кандидата имя автора
>   убирается (ПД третьих лиц). Генеративная секция — промпт `summary.points`, ответ со словами решения о
>   человеке («рекомендую найняти», «відмовити») отклоняется. Строка «Документ сформовано автоматично… Оцінки
>   програми перевірено людиною: так/ні» — в `body.disclaimer` на языке документа и в письме
>   `interview_result_ready`; секции её не выключают.
> - **§7.15.** Срок назначает сверка `summary.auto_send` (каждые 10 минут), а не запись оценки: задержка
>   отсчитывается от момента, когда рекрутер получает `summary_auto_send_scheduled` и может отменить.
>   Авто-отправка однократна на кандидата: если документ ему уже уходил, назначен или отменён, новую версию
>   отправляет человек. В момент срока проверки §7.15 выполняются заново. Нет документа у кандидата с
>   оценкой не ниже порога — он собирается, срок назначит следующий круг.
> - **§7.16.** `ai.quality_sample` (06:00): 5 % по хешу идентификатора (повторный прогон дублей не даёт),
>   все `major`, все с уверенностью ниже 0,5, окно — неделя. `ai.metrics_rollup` (06:30): доля `major` за 30
>   дней по паре `prompt_key` + `prompt_version`; порог 15 % считается от 10 решений (иначе один `major` из трёх
>   давал бы тревогу от шума).
...
> [дополнено, PR-29] Коды — `snake_case`, как весь реестр: `interview_result_ready` (кандидату, e-mail, в
> окне кандидата 09:00–20:00; в тексте — «Документ сформовано автоматично»), `summary_auto_send_scheduled`
> (рекрутеру карточки, иначе администраторам, колокольчик), `ai_review_hint_failed` (администраторам,
> колокольчик, раз в сутки), `ai_quality_degraded` (администраторам, e-mail и колокольчик, раз в сутки на
> версию промпта).
...
> [исправлено, PR-28] Ранее: «`GET /interviews/:sessionId` … `409 interview_consent.required`», «`POST
> /interviews/:sessionId/consent`», «`POST /interviews/:sessionId/start`». До согласия сессии нет (к. 1,
> к. 2), поэтому вход, согласие и старт адресуются тестом: `GET /interviews/entry/:quizId` (текст
> согласия с редакцией и хешем, что показать дальше), `POST /interviews/entry/:quizId/consent`
> (`{decision, textVersion, textHash, alternative?, preferredTime?}`), `POST …/alternative`
> (`{reason: ai_unavailable | no_microphone}`), `POST …/start` (`{answerMode}`), `POST …/text-form`;
> письменная форма — `GET /interviews/text-form/:attemptId`, `PUT …/answers/:questionId`, `POST …/submit`.
> Сессия: `GET /interviews/:sessionId`, `POST …/heartbeat`, `POST …/pause`, `POST …/turns/:ordinal/upload`
> (`{mime, bytes}` → ссылка на загрузку, файл — в S3 напрямую), `POST …/turns/:ordinal/answer`,
> `POST …/finish`, `POST …/withdraw` (`204`). Коды согласия — в своём пространстве (`41` §8.2.1, §8.2.3):
> `409 interview_consent.required`, `409 interview_consent.already_decided`, `422 interview_consent.invalid`;
> исчерпание оси на старте — единый `409 limit_exceeded`, истёкший ИИ — `409 ai.unavailable`, оба с
> `details.alternative`. Сценарии: `GET|POST /interview-scenarios`, `GET|PUT /interview-scenarios/:id`
> (статус — полем `status`; правка опубликованного — новая версия, `versionCreated`),
> `GET|POST /interview-scenarios/:id/criteria`, `PUT|DELETE …/criteria/:criterionId`. Ручки кандидата
> прав не требуют (`41` §2.3): тест должен быть назначен ему, сессия — принадлежать ему; чужое — `404`.
> Всё под `/interviews*` и `/interview-scenarios*` гасится вместе с рекрутингом (`candidates.disabled`).

> [дополнено, PR-29] Реализовано: `POST /candidates/:id/interview/criteria/:criterionId/override`
> (`interview.override`; `409 session.not_scored`, `409 conflict` с `details.current`, `422` вне шкалы);
> `GET|POST /candidate-summaries`, `GET|PATCH /candidate-summaries/:id` (`summary.view` / `summary.edit`;
> `409 summary.no_data`, `409 summary.sent`), `POST …/:id/send` `{channel: email|link}` и `…/:id/revoke`
> `{reason}` (`summary.send`; `422 contact.missing`, `409 consent.withdrawn`, `409 summary.not_latest`,
> `409 summary.not_candidate`), `POST …/:id/auto-send/cancel` (`409 summary.auto_send_not_scheduled`);
> публичная ссылка — **`GET /api/v1/public/candidate-summaries/:token`** в публичном контуре (`41` §8.3.2,
> `44` В-9): тенант из токена функцией `candidate_summary_public_lookup`, `hitRateLimit` 30 за 10 минут,
> неизвестный токен — `404` с выровненным временем, истёкший — `410 summary.share_expired`, отозванный —
> `410 summary.revoked`; страница кандидата — `/summary/:token`. `GET /review-hints/:targetKind/:targetId`
> (`ai.review.use`; `404 hint.absent`, `409 hint.degraded`, `403` — работа в чужих руках).
...
> [дополнено, PR-28] Работают `interview.transcribe` и `interview.score` (по событию, задача несёт
> `tenantId`; очередь без своих повторов — повторы с отсрочкой ставит сервис) и `interview.reap` (каждые
> 15 минут, круг по тенантам с включённым рекрутингом): кандидату — `interview_abandoned` (e-mail),
> рекрутеру — `interview_abandoned_recruiter`; брошенная сессия без ответов возвращает резерв ИИ.
> `ai.calls_cleanup` теперь отправляет в корзину и файл входа.

> [дополнено, PR-29] Работают `interview.media_purge` (03:40, **все** тенанты, отказ S3 — падение задачи),
> `summary.build` (по событию: собеседование `scored` или `needs_human`, у кандидата ещё нет Підсумка),
