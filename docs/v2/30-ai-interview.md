...
18. **Идемпотентность.** Каждый вызов идёт с `Idempotency-Key` = `sha256(tenant_id, ref_kind, ref_id, prompt_version, try_no)`; повтор с тем же ключом возвращает сохранённый `ai_calls.output` и не тратит лимит.
