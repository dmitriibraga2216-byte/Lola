...
и публикация курса его не перезакрепляет. Узел траектории — место использования тем же
способом: `trajectory_nodes.library_version_id` (PR-26).

> [исправлено, PR-26: (1) узел-ссылка — узел-задание, чей контент — материал-тело модуля
> (`content_type = 'resource'`, `content_id` — материал), а какой снимок получит человек, решает
> `library_version_id`: назначение узла закрепляется за снимком этой версии
> (`assignments.subject_version_id`), как у урока — `resource_version_id`, и ученик читает тело
> модуля только по такому назначению (`resources.ts#viewResource`); (2) плюс CHECK
> `trajectory_nodes_library_ref_ck` (`library_version_id is null or (kind = 'task' and
> content_type = 'resource' and content_id is not null)`) — зеркало `lessons_library_ref_ck`;
> (3) имя внешнего ключа задано явно — `trajectory_nodes_library_version_fk`: сгенерированное длиннее
> 63 символов; (4) активные места узлов PR-25 (ссылка жила только в реестре) перенесены в колонку
> той же миграцией `0088`, и узел стал читать тело своей версии] Ранее: «Узел траектории станет
> местом использования тем же способом в PR-26».
...
> [исправлено, PR-26: у полотна Lola нет контекстного меню узла — его роль играет панель
> выбранного блока (`docs/17` §5.1), поэтому у палитры два входа: «+ Бібліотека модулів ▸» над
> полотном (новый блок-ссылка) и «Замінити модулем бібліотеки ▸» в панели задания. Выбор — такая же
> локальная правка полотна, как любая другая: версия закрепляется при «Зберегти» (сервер вставляет
> модуль той же транзакцией, что и граф, и откатывает всё сохранение, если вставить нельзя —
> `PUT /trajectories/:id/graph` с `libraryModuleId`). «8 последних использованных» — сначала
> вставленные самим автором, затем коллегами. Вместо «Відкрити в бібліотеці» — «Переглянути v2»
> (тело закреплённой версии в окне): экрана карточки модуля (§5.2) ещё нет, он приходит PR экранов]
> Ранее: «Контекстное меню узла (05-tracks.md §5.4) открывает палитру», кнопка «Відкрити в
> бібліотеці».
...
> [исправлено, PR-26: «N людей, які вже почали» у трека — прохождения `in_progress`, которым узел
> уже выдал задание на закреплённой версии: кто до узла ещё не дошёл, получит новую версию, и
> обещать ему «залишиться на v2» было бы неправдой; у курса — записи `in_progress` (они на своей
> версии курса). При нуле строка — «Ще ніхто не почав цей блок — усі отримають v4». Данные диалога
> считает сервер (`GET /library/usages/:id/update-preview`), diff — прямое сравнение закреплённой и
> целевой версии, а не цепочка сохранённых diff соседних версий] Ранее: одна строка «Оновлення не
> змінить проходження 12 людей, які вже почали. Вони залишаться на v2».
...
3. **Обновление — только явное.** Кнопка «Оновити до останньої версії» (§5.5) доступна носителю
   ...
   > [исправлено, PR-26: узел траектории обновляется и в опубликованной траектории — состав графа
   > не меняется, а выданные узлом назначения закреплены за прежним снимком; урок курса — только
   > в черновой версии курса (опубликованная неизменяема, `docs/11` §7.1, на неё ссылаются
   > прохождения и сертификаты, §12), иначе `409 container.published` — редактор курса сам создаёт
   > черновик, и место переходит за ним. Только вперёд: та же версия — `409 already_latest`, назад —
   > `422 version_downgrade` (§12: отката версии нет), выведенная из оборота — `422 version_retired`.
   > Массовое «Оновити все» (`library.manage`) людей в процессе не проверяет — действие явное, как и
   > одиночное; урок опубликованной версии курса оно пропускает (`skipped[]`)] Ранее: без оговорок
   > о контейнере и направлении.
4. **Исключение — `is_hotfix` `[решение] Р-31.4`.** Версия с флагом «Критичне виправлення»
   ...
   > [исправлено, PR-26: «в статусе in_progress по контейнеру» — прохождения траектории
   > (`trajectory_enrollments`) или записи на курс (`enrollments`). Урок курса в опубликованной
   > версии хотфикс не меняет и без людей в процессе — версия курса неизменяема (§7.3): место
   > остаётся `stale`, автору тот же `library_hotfix_blocked` с причиной «опубліковану версію курсу
   > не змінюють»; `fixed` хотфикс сам не получает. Разносит фоновая `library.hotfix_propagate`
   > сразу после публикации (`libraryUsages.ts#propagateHotfix`), решение по месту —
   > `shared/domain/library.ts#hotfixDecision`; уведомления — по одному на контейнер и адресата]
   > Ранее: без уточнения контейнеров и курса.
...
7. **Тип и иконка `[решение] Р-31.5`.** `content_kind` берётся из перечня `resources.kind`
   ...
   > [исправлено, PR-26: блока `table` в репозитории нет (`shared/schemas/content.ts`, десять типов
   > — перечень `11` §3.3 реализован не целиком), таблица живёт в HTML блока `text`, санитайзер её
   > пропускает. Признак таблицы — блок `table` (если появится) или `<table` в HTML текстового блока.
   > Тело — последней версии, пока версий нет — черновика; иконку считает сервер (`typeIcon`
   > карточки, `shared/domain/library.ts#typeIconOf`), `content_kind` не меняется (критерий 7);
   > `scorm` в перечне нет (§3.2)] Ранее: «иначе есть блок `table` → таблица».
8. **Поиск.** GIN по `search_tsv` (название, описание, метки) с ранжированием `ts_rank`; плюс
   ...
   шлюз модели `server/services/ai/gateway.ts` — профиль роли `embed` тенанта (`docs/v2/30` §3.2),
   по умолчанию детерминированная заглушка (`HANDOFF` §6). Векторная часть запроса и RRF — PR-26.
   > [исправлено, PR-27: каждый вызов модели — строка `ai_calls`, `30` §7.16] Ранее: «через
   > `server/services/embeddings.ts` — OpenAI-совместимый провайдер при `EMBEDDINGS_URL`, без ключа
   > детерминированная заглушка». Провайдера выбирает профиль, а не переменная окружения; вектор
   > модуля (батч — одна строка журнала) и вектор поискового запроса (`ref_kind = 'search_query'`)
   > идут через шлюз, вне тарифа (эмбеддинг — индекс поиска, а не покупаемая функция ИИ, `35` §7.4
   > поиск не блокирует). Метка модели — `driver:model:dims` профиля, который реально ответил;
   > у заглушки она прежняя, `stub:hash-v1:768`, и векторы PR-25 не пересчитываются.
   > [исправлено, PR-26: (1) полнотекст ищет и по телу **последней опубликованной версии** — вес C в
   > `search_tsv` (триггер миграции 0088 срабатывает и на смену `current_version_id`): критерий 8 —
   > запрос из одного слова, а гибрид по тексту выше включается только с четырёх, и без тела в
   > полнотексте «розведення» не находилось бы; черновик по-прежнему не индексируется; (2) запрос
   > префиксный (`розвед:*`) — палитра ищет по мере набора; (3) векторная часть берёт только векторы
   > текущей модели (`embedding_model`) и отсекает косинусную близость ниже 0,2 — иначе вектор
   > «находит» любой модуль и «Нічого не знайшли» (§5.1) не показалось бы никогда; (4) RRF с k = 60,
   > при равенстве — порядок полнотекста; (5) с `q` список — одна страница по релевантности без
   > курсора (`limit` ≤ 50)] Ранее: «GIN по `search_tsv` (название, описание, метки)», векторная
   > часть и RRF не реализованы.
...
10. **Отвязка в копию.** «Відʼєднати і зробити копією» создаёт обычный `lessons` с телом
    ...
    > [исправлено, PR-26: у узла траектории `makeCopy` есть и по умолчанию включён: узел получает
    > **опубликованный** материал-копию с телом закреплённой версии — у траектории нет своей
    > публикации контента, и узел назначает то, что уже можно выдать; без `makeCopy` — пустое
    > задание, в которое автор выберет другой контент. Опубликованный трек не отвязывается
    > (`409 container.published`), как и не принимает вставку] Ранее: «`makeCopy` для узла появится
    > вместе с `trajectory_nodes.library_version_id`».
...
`server/services/notifications.ts`, подписи — `notif.code.*` в трёх словарях). PR-26 — ещё три:
`library_hotfix_applied`, `library_hotfix_blocked`, `library_stale_digest`, все только в колокольчик.

> [исправлено, PR-26: в `library_hotfix_applied` снято «треку» — место бывает и уроком курса:
> «У «{container}» модуль «{title}» оновлено автоматично: критичне виправлення»; в
> `library_hotfix_blocked` причина одна из двух — «люди вже проходять» (текст таблицы дословно) или
> «опубліковану версію курсу не змінюють» (§7.4)] Ранее: «Коды хотфикса и дайджест устаревших
> ссылок приходят с PR-26», шаблон «У треку «{container}» …».
...
> [исправлено, PR-26: (1) ещё две ручки — `GET /library/modules/recent` (палитра §5.4: «до 8
> последних использованных») и `GET /library/usages/:id/update-preview?toVersion` (диалог §5.5:
> changelog пропущенных версий, поблочный diff, тела «було / стало» и число людей, которые уже
> начали, — всё считает сервер); (2) `update-version` принимает `{toVersion?}` (без него — последняя),
> коды: `409 already_latest`, `422 version_downgrade` (отката нет, §12), `422 version_retired`,
> `409 container.published` (урок опубликованной версии курса), `404 not_found` для места или
> версии; (3) `update-all-usages` — `{toVersion?}` → `{updated, skipped[], toVersion}`, пропуск — место,
> которое обновить нельзя; (4) `diff` — `GET …/versions/:from/diff/:to` → `{from, to, changelogs,
> diff, before, after}`, одна и та же версия — `422 same_version`; (5) `detach` у узла трека —
> `{makeCopy}` (по умолчанию `true`), ответ `{lessonId, resourceId}`; (6) узел из палитры сохраняется
> вместе с полотном — `PUT /trajectories/:id/graph`, задание с `libraryModuleId` (`docs/04`); отказ
> вставки откатывает сохранение и отвечает кодом библиотеки с `details.nodeRef`] Ранее: ручек и
> кодов этого пункта в таблице не было.
...
> [исправлено, PR-26: `library.hotfix_propagate` ставится публикацией версии с `is_hotfix`
> (`singletonKey` — версия), повторный прогон ничего не дублирует; `library.stale_digest` —
> понедельник 09:00 **по времени Киева**, как остальные суточные сканы репозитория, а не по таймзоне
> точки: у автора трека точек бывает несколько, а дайджест — один в неделю (ключ дедупликации —
> неделя); сумма — все устаревшие активные места в его треках и курсах. Сверка `usage_recalc`
> закрывает и место узла, потерявшего `library_version_id` мимо сервиса] Ранее: «понедельник 09:00
> по таймзоне точки».
