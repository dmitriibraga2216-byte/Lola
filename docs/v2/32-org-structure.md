...
7. **Снимок** делается перед импортом, перед массовым перемещением (>20 узлов) и ежедневно
   ...
   `kind='org_structure'`), ключ идемпотентности — `external_key`: план сначала решает, у кого
   какой родитель, и выводит из этого пути всех узлов, узлы пишутся родителями вперёд, колонки —
   §9. **Витрина кэшируется** на 5 минут.

   > [исправлено, реализация PR-31: (1) вид импорта пишется в колонку `import_jobs.kind`, она
   > у таблицы уже есть — история импорта людей фильтрует `kind='users'`, аудитория заданий
   > пишет `task_audience`; `options.entity` дублировал бы тот же факт вторым местом, а строка
   > с `kind` по умолчанию (`users`) попала бы в историю и в «Застосувати» импорта людей;
   > (2) «сначала все узлы, затем родители» заменено раскладкой по итоговому дереву
   > (`shared/domain/orgLayout.ts`): петля, висячий узел и 13-й уровень отсекаются **до** записи,
   > построчно, и уходят в `org_conflicts` (§12 п. 3), а не ловятся триггером на коммите;
   > (3) снимок «до отката» — вид `pre_bulk_move`: откат переставляет всё дерево, это и есть
   > массовое перемещение, нового вида снимка не заводится (правило 13 корневого `CLAUDE.md`);
   > (4) `[решение]` импорт **добавляет и уточняет** привязки людей, но не снимает тех, кого в
   > файле нет: файл «только дерево» не должен опустошить структуру, снять человека — действие
   > конструктора; исключение — именной узел, где человек из файла заменяет прежнего держателя,
   > иначе узел остался бы с двумя; (5) `[решение]` возвращённое откатом назначение — новая
   > строка от даты отката: закрытое назначение обратно не открывается (§4), история остаётся
   > правдивой, а пара «узел × человек», роль и признак основного совпадают со снимком]
   > Ранее: «`options.entity='org_structure'`), ключ идемпотентности — `external_key`: сначала
   > создаются все узлы, затем связываются родители»
...
`employee_external_id` · `assignment_role` · `assignment_started_at` · `sort` ·
`assignment_is_primary`.

> [исправлено, реализация PR-31: (1) четырнадцатая колонка `assignment_is_primary` — без неё
> совместительство не переживает «экспорт → импорт»: основное подчинение одно на человека
> (§7 п. 4), и по одним строкам не понять, какое из двух назначений основное; пусто —
> основным считается первое упоминание человека, остальные — совместительством с
> предупреждением; (2) узел с несколькими держателями — несколькими строками с тем же
> `external_key`: колонки узла повторяются (у строки-держателя их можно оставить пустыми, а
> заполненные обязаны совпадать), колонки назначения — свои; (3) ключ узла, созданного в
> конструкторе без `external_key`, — его id: импорт находит узел и по нему; (4)
> `employee_external_id` — зовнішній № человека, а если его нет — телефон: тот же порядок
> идентификации, что у импорта людей (`docs/06` §6.5.6); ищутся только сотрудники (правило 17);
> (5) значение, начинающееся с `=`, `+`, `-`, `@`, выгружается с ведущим апострофом (защита от
> формул в табличном редакторе), импорт этот апостроф снимает] Ранее: список из тринадцати
> колонок, без правил для нескольких держателей
...
> [исправлено, реализация PR-31: (1) **откат идёт в запросе** одной транзакцией и отвечает
> итогом (`nodes`, `archived`, `assignmentsCreated`, `assignmentsEnded`, `assignmentsUpdated`,
> `dismissedSkipped`, `preSnapshotId`), а не `{job_id}`: работа ограничена размером снимка и
> сделана пачками, результат администратор должен увидеть сразу, а хранилища статуса у отката
> нет (`import_jobs` — это импорт); второй одновременный откат — `409 rollback_in_progress`
> (транзакционная блокировка тенанта), откат во время применения импорта — `409
> import_in_progress`, битый снимок — `422 snapshot_invalid`; (2) **импорт — четыре пути**:
> `POST /import` (multipart, файл) — разбор, сопоставление колонок по заголовкам и
> предпросмотр; `GET /import/:id` — предпросмотр и ход применения; `POST /import/:id/mapping`
> — сопоставление и опции, повторная проверка тех же строк (`422 mapping_invalid`); `POST
> /import/:id/apply` — запуск фоновой задачей `org.import_apply`, ответ `{jobId}` (`409
> import_in_progress` — другой импорт ещё применяется, `422 nothing_to_apply` — в файле нет
> изменений); ошибки файла — `422 file_invalid`, `file_encoding`, `file_empty`,
> `too_many_rows`, `file_too_large`; (3) список снимков — ключевой курсор (`?cursor&limit`,
> ответ `{rows, cursor}`); (4) `429 too_many_snapshots` не заведён: снимки делаются
> администратором и перед реорганизациями, лимита в документе нет]
> Ранее: строка выше как единственное описание импорта и отката
...
> [исправлено, реализация PR-31: фоновой задачей идёт только `org.import_apply` — весь файл
> одной транзакцией под блокировкой «одна реорганизация на тенант», без повторов (упавший
> импорт закрывается `failed` с письмом `import_failed`, дерево не тронуто). Откат — в запросе
> (§10), отдельной задачи `org.rollback_apply` нет] Ранее: «`org.import_apply` \|
> `org.rollback_apply` — по запуску»
...
3. Импорт принёс неизвестного родителя: строка в ошибки («Батьківський вузол {{key}}
   ...
   `manager_cycle` с `source='import'`.

   > [исправлено, реализация PR-31: петля длиннее одного звена — `manager_cycle` («Кільце
   > керівників»): вид есть в общем перечне из десяти (`44` В-7), а пакетный список, от которого
   > писался пункт, вида «кольцо» не знал. `manager_self` остаётся за узлом, подчинённым самому
   > себе (`parent_external_key` = свой ключ). Висячий узел — родителя нет в файле и в дереве,
   > он в архиве или сам отклонён — `unit_missing`; 13-й уровень — `depth_exceeded`. Все —
   > с `import_job_id`, в дерево ни один не попадает] Ранее: «конфликт `manager_self` с
   > `source='import'`»
