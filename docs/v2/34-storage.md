...
> [исправлено фазой 3, реализовано PR-36 — миграция `0083_v2_storage_quota.sql`] В DDL выше у
> `storage_usage_counters` и `storage_usage_daily` колонка `category_id → course_categories`
> заменена на `stage_code text` с тем же перечнем, что у `media_assets.stage_code` (решение `44`
> В-10): `null` — девятый ключ `other`, `unique nulls not distinct` сохранён. Добавлено сверх DDL:
> `storage_retention_policies.trash_days` (срок корзины — `44` §8 требует держать его строкой
> политики, а не константой в коде; умолчание 30), `storage_deletion_requests.reason` (форма §6.1
> требует причину, DDL хранил только фразу), `storage_pending_uploads.media_id` (без связи с
> файлом, заведённым под досылку, подтверждение загрузки не находит сдачу), check-и перечней
> `anchor`, `action`, `mode`, `status` из `shared/enums.ts`. `requested_by` и `updated_by` —
> `on delete set null`, а не блокирующий FK: стирание ПД человека (§7.6 п. 2) не упирается в его
> заявки. `origin` в новых таблицах check-ом **не** закрыт: перечень один — на `media_assets`
> (В-6, инвариант 19); счётчики получают значение только триггером, политики — через zod.
> Счётчик ведёт триггер `storage_counter_apply` (§7.4 п. 1): `AFTER INSERT OR DELETE` и
> `AFTER UPDATE OF lifecycle, bytes, origin, stage_code`.
...
> [уточнено, реализовано PR-36] Путь — `/admin/settings/storage`, корзина и политики —
> вкладки `/admin/settings/storage/trash` и `/admin/settings/storage/policies`, а не `/storage*`:
> тенантные экраны этого репозитория живут под `/admin/settings/*` (так же решено для
> `/admin/settings/billing`, `35` §5). Не сделано в PR-36 (API готов, экран — нет): фильтры
> «Категорія», «Трек», «Співробітник» (`categoryId`, `courseId`, `userId` в `GET /storage/files`),
> превью image/video в строке и меню «…». «Збільшити сховище» не форма докупки: платёжного
> провайдера нет (`44` §8), кнопка объясняет, что объём подключает менеджер Lola, — оператор
> выдаёт `storage_pack` в панели `/ops`, и ждущие записи получают место сразу. Вид `manager`
> («свои точки») не сделан: `storage.view` пока только у `admin` и `owner` (`39` П-01).
...
> [уточнено, реализовано PR-36] «Очистити зараз» в корзине нет: до ответа владельца продукта
> об удалении объектов (`44` §8) досрочная очистка значила бы только «лишить права
> восстановить» — квота свободна с момента удаления (§7.2 п. 3). Срок «30 днів» в пустом
> состоянии и в форме §6.1 — не константа экрана, а `trash_days` политики хранения.
...
1. Основная ось — `media_assets.origin`, задаётся **при выдаче presigned URL**, а не вычисляется потом: клиент обязан передать
   ...
   **не пересчитывается** при смене этапа у курса: разбивка должна быть стабильна во времени. Файл вне курса ключа не получает
   ...
   единственном входе загрузки; backfill уже загруженных файлов — PR-36 вместе с экраном и сводкой]` `[исполнено PR-36: SQL-функция
   storage_classify_backfill() вызвана миграцией для всех тенантов; повтор — сервисом classifyBackfill() под RLS
   тенанта. Ссылки ищутся и по id файла, и по ключу объекта: обложки в продукте хранят то одно, то другое]`
...
> [исправлено, PR-36] Из запретов п. 1 не реализованы два. «Файлы со ссылкой из `audit_log` за
> последние 12 месяцев»: с PR-11 каждая загрузка пишет `media.upload` с `entity_id` файла, и правило
> буквально запретило бы удалять любой файл младше года; его намерение — не терять след удалённого
> доказательства — закрыто подтверждением §6.1 и записями `media.delete` / `storage.bulk_delete`.
> «Файлы попытки с открытой апелляцией» — сущности апелляции в продукте нет. «Сдача в статусе
> `review` или `reworking`» — это статусы `submitted`, `in_review`, `rework` таблицы
> `workshop_submissions`: одиночное удаление отвечает `409 under_review` (§12), заявка пропускает
> файл с `reason='under_review'`. Признак доказательства (п. 2 §7.1) выставляет решение наставника
> `accepted` / `rejected` по сдаче практикума.
...
> [уточнено, реализовано PR-36] Отложенная загрузка идёт **единственным входом** `POST /media/upload-url`
> (решение `44` В-17, `upload-intent` отменён): запрос с `clientRef` (ключ записи на устройстве)
> при исчерпанной квоте отвечает `202 {deferred: true, pendingId, expiresAt}` вместо
> `400 media.storage_limit`, повтор с тем же `clientRef` досылает файл, когда место выдано.
> Хранилище на устройстве — IndexedDB (OPFS доступен не во всех браузерах телефонов);
> устройство синхронизируется при открытии кабинета, раз в 5 минут и при появлении сети.
> Сотрудник без `media.upload` (у `employee` его нет, `01` §1.4) загружает свою работу по
> `learn.attempt` — только происхождения `workshop_submission`, `video_answer`, `issue_screenshot`;
> без этого критерий §13 к. 1 для роли `employee` был недостижим. Файл-обещание в сдаче —
> `{pendingId}`; при досылке заменяется на `{mediaId}`, по истечении — `{pendingId, lost: true}`.
> Метка «Очікує вивантаження» и запрет взять работу — в очереди практикумов (`/review/workshops`,
> `409 pending_upload`); единая очередь `/review/queue` метки пока не показывает. Срок проверки
> наставника отсчитывается от досылки файла, а не от сдачи: пока файла не было, взять работу было
> нельзя. Внеочередной проход `storage.pending_upload_retry` — после выдачи опции, правки лимита
> оператором и удаления файлов.
...
> [уточнено, реализовано PR-36] Заведены `storage_upload_blocked`, `storage_upload_deferred`,
> `storage_pending_upload_done`, `storage_pending_upload_expired`, `storage_bulk_delete_done`.
...
> [уточнено, реализовано PR-36] Реализованы: `GET /storage/summary` (`groupBy=origin|stage` —
> второй режим по этапам, В-10; в ответе ещё `stageLabels`, `trashDays`, `otherShare`, `otherFiles`,
> корзина и ожидающие загрузки), `GET /storage/files`, `/storage/trash`, `/storage/pending-uploads`,
> `POST /storage/files/:id/restore` (`409 already_purged`, `409 not_deleted`), `POST /storage/deletions`,
> `/:id/confirm` (`422 reason_required`, `422 confirm_phrase_mismatch`, `409 deletion.not_draft`),
> `/:id/cancel`, `GET`/`PUT /storage/retention-policies` (`422 dry_run_required`,
> `422 evidence_ack_required`, `422 keep_months_range`), `POST /storage/retention-policies/dry-run`.
> `POST /media/upload-url` — `clientRef` и ответ `202 {deferred}`, `409 pending_upload_done`,
> `410 pending_upload_abandoned`; `DELETE /media/:id` — плюс `409 under_review`. Не сделаны:
> `GET /storage/files/:id` и `…/download` (карточку и ссылку отдаёт `GET /media/:id`, ссылка на
> доказательство — 120 с, В-19), `/storage/addons` (платёжного провайдера нет, `44` §8),
> `GET /reports/storage`, `Idempotency-Key` (в продукте его нет ни у одной ручки; подтверждение и
> отмена заявки идемпотентны по статусу).
...
> [уточнено, реализовано PR-36] `storage.purge` — 04:00 по Києву, партиями по 1000: `pending_delete`
> с истёкшим `purge_after` → `purged`, одна запись `storage.file.purge` на партию. **Объект в S3 не
> удаляется** (`44` §8): безвозвратное удаление — вопрос владельца продукта, до ответа список к
> физическому удалению — строки `lifecycle='purged'` (их `key`). `storage.pending_upload_retry` —
> каждые 15 минут. `storage.counter_reconcile` — внутри `usage.collect`, он же наполняет
> `tenant_usage.storage_by_category` девятью ключами. `storage.classify_backfill` — SQL-функция
> миграции. `storage.bulk_delete` исполняется синхронно в подтверждении заявки, теми же партиями по
...
> `interview.media_purge` (03:40, до корзины) и только после подтверждения S3 ставит `purged`; восстановить
> такую запись из корзины нельзя — `409 storage.not_restorable` (туда её кладут отзыв согласия, перезапись и
> ответ текстом). Решение `44` §8 об остальных файлах этим не меняется: для голоса физическое удаление
> предписано самим документом и обещано кандидату на экране согласия.
...
> [уточнено, PR-36] Критерии 1, 3, 4, 7 закрыты PR-36 и проверены
> `tests/integration/v2-storage-quota.spec.ts`; 2, 6 — PR-11, 5 — PR-11 (первая половина) и PR-36
> (дрейф, вторая половина). Критерий 3 «объекта в S3 нет» выполняется **до ответа владельца
> продукта** частично: строка `purged`, восстановить нельзя, квота свободна, сдача показывает
> «Файл видалено», а объект остаётся в бакете и входит в список к физическому удалению (`44` §8,
> условие входа PR-36). Критерий 1 «в течение 15 минут файл в S3» — место выдаётся сразу после
> выдачи опции оператором (внеочередной проход), файл досылает устройство при ближайшей
> синхронизации (открытый кабинет — не реже раза в 5 минут).
