...
>
> [исправлено, fix-candidate-limit-race] Та же гонка была у оси `candidates_active`: создание кандидата и отклик
> по ссылке вакансии проверяли ось отдельным подключением до транзакции, возврат в воронку и перенос в колонку
> `maps_to = 'active'` (поштучно и пачкой) — не проверяли вовсе. Теперь каждый такой путь зовёт
> `assertCandidatesWithinLimit()` — то же тело, что у мест сотрудников (своя блокировка оси, подсчёт в
> транзакции, fail-closed, `409 limit_exceeded` с `axis: 'candidates_active'`). Найм кандидата место кандидата
> только освобождает. Подробно — `docs/28` §28.25, тесты — `tests/integration/candidate-limit.spec.ts`.
