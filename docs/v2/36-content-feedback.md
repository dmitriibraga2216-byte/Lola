...
> [исправлено, PR-24: скоупы разбора выданы ролям по этой таблице — методисту `content_issue.triage`
> (к `view` из PR-23), керівнику точки `content_issue.view`. Правка правил маршрутизации требует
> `content_issue.assign` — строка «Править маршрутизацию» выше отдаёт её только администратору,
> а `41` §3 записывал все четыре пути правил на `triage`, то есть и методисту; чтение правил
> оставлено на `triage`: автор видит, почему карточка пришла к нему] Ранее: скоуп правки правил не
> отделён от разбора
...
> [исправлено, PR-23: идентификатор блока в `resources.body` — короткая строка редактора
> (`shared/schemas/content.ts`, `blockBase.id`: 1–64 символа), а не uuid; колонка `uuid`
> не приняла бы ни одного реального значения] Ранее: «`issues.block_id` | uuid»
...
| `issues.block_id` | text | — | null | есть в `body` | блок внутри материала (`11` §3.3) — чтобы автор понял, какой абзац |
...
create table content_issues (
  ...
  target_id uuid not null, block_id text,
...
> [исправлено, PR-24, миграция `0077_v2_content_routing`: у `content_issue_routing_rules` — ещё
> `updated_at` (общие колонки всех таблиц репозитория), проверки `sort between 0 and 10000` и
> допустимых `target_type`/`issue_type`, частичный уникальный индекс `(tenant_id) where fallback` —
> запасное правило одно (§6.3). Две колонки базовых таблиц, без которых §7.5 и §7.8 невыполнимы:
> `course_categories.owner_id uuid references users(id) on delete set null` — «владелец категории
> курса» шага (в) маршрутизации, и `attempt_results.issue_id uuid references content_issues(id)
> on delete set null` — «новая запись результата со ссылкой `issue_id`» (§7.8)] Ранее: DDL без них
...
> [исправлено, PR-24: уточнения таблицы переходов, по которым работает `checkTransition()`
> (`shared/domain/contentIssues.ts`). (1) `new → in_progress` делает человек — «Взяти в роботу»,
> себе, если ответственного ещё нет; назначение правилом §7.5 ставит ответственного, но статус не
> меняет: иначе вкладка «Нові» пустовала бы всегда. Исключение — автор, пожаловавшийся на свой
> материал (§12): карточка сразу `in_progress` на нём. (2) Добавлен `deferred → in_progress`
> (автор, админ) — вернуть отложенную; без него отложенная карточка выходила бы только отказом.
> (3) `* → rejected` — из `new`, `in_progress`, `deferred`; резолюция — одна из четырёх отказных
> (`not_an_error`, `duplicate`, `wont_fix`, `spam`). (4) Повторная отправка того же статуса —
> 409 `content_issue.invalid_transition` с актуальной карточкой: так второй из двух одновременных
> разборов узнаёт, что его опередили (§12). (5) Ручное `fixed → closed` невозможно, пока баллы
> ждут пересчёта (`rescore_state` = `needed`|`in_progress`, §7.7 в)] Ранее: таблица без этих строк
...
> [исправлено, PR-24: «Коментар для заявника» обязателен при **любом** отказе — у всех четырёх
> резолюций отказа, включая `duplicate` и `spam`, которых в радио выше нет, но которые есть в
> перечне §3.2: заявителю уходит `content_issue_rejected` с текстом «Ми перевірили: {{comment}}»,
> и пустым он быть не может. «Три последних значения» радио включали бы «Питання анульовано» —
> подтверждение, а не отказ; там комментарий необязателен. Совпадает с §3.1 и §4 (`* → rejected`
> — обязателен `resolution_comment`)] Ранее: «✓ при трёх последних значениях»
...
5. **Маршрутизация.** По порядку: (а) первое активное правило по `sort`, совпавшее по `target_type`, `issue_type`,
   ...
   > [исправлено, PR-24: как это устроено (`server/services/contentIssueRouting.ts`). «Неактивен»
   > не определяется заново: это отметка офбординга PR-07 (`users.status = 'archived'`) и блокировка
   > входа — то же условие, по которому считается ось лимита `users_active`
   > (`ACTIVE_EMPLOYEES_ONLY()` в `server/services/repo/people.ts`). Каждый шаг пропускает
   > неактивных: правило, адресованное уволенному, не останавливает поиск, а уступает следующему.
   > Правило на роль отдаёт карточку наименее загруженному действующему носителю роли. Категории
   > элемента — категории курсов, где он встречается (`course_ids` выводятся через уроки всех версий
   > курса, а не только из урока заявителя), плюс категория статьи базы знаний или банка вопроса.
   > «Владелец категории» — новая колонка `course_categories.owner_id` (Г-36.10). Запасное правило
   > фильтров не имеет и в шаге (а) не участвует] Ранее: шаги без правила «неактивен»
...
8. **Пересчёт результатов после подтверждения** `[решение]` — продолжение кнопки **«Перерахувати»** из отчёта
   ...
   > [исправлено, PR-24: (1) «`submitted`/`graded`» — в репозитории это статусы `submitted`,
   > `review`, `passed`, `failed`, `expired` (`RECALCULABLE_ATTEMPT_STATUSES`, те же у «Перерахувати»
   > теста); `in_progress` и `annulled` не трогаются. (2) «Только улучшение» — это и падение балла
   > без смены статуса: такая попытка тоже остаётся как есть и попадает в список «с ухудшением»
   > (`rescoreVerdict()`). (3) Резолюции `question_fixed`/`question_void` сами ставят
   > `affects_scoring = true` и `rescore_state = needed`, если жалоба пришла не из попытки;
   > `content_issue_rescore_ready` уходит в момент подтверждения, а не при подаче: до исправления
   > «можна перерахувати» неправда. (4) «Не перераховувати» — режим `skip` того же
   > `POST /content-issues/:id/rescore`, причина обязательна и становится ответом заявителям.
   > (5) Пересчёт идёт синхронно партиями по 200 попыток, каждая — своей транзакцией; прерванный
   > продолжается с места обрыва (`rescore_state = in_progress` старше 10 минут), уже пересчитанные
   > пропускаются по `attempt_results.issue_id`. Фоновый предпросмотр для тысяч попыток (§12) — не
   > сделан, предпросмотр считается сразу] Ранее: без этих уточнений
9. **Что считается закрытием.** Карточка закрывается, только когда: стоит `resolution`; для `fixed`/`question_fixed`
   ...
   > [исправлено, PR-24: «опубликована версия выше `content_version`» — по элементам
   > (`fixPublished()`): материал и блок — опубликованная редакция `resources.version`; вопрос —
   > `questions.version` (у вопроса нет черновика: правка сразу идёт в новые попытки и она и есть
   > публикация); урок — опубликованная версия курса (публикация курса копирует уроки в новую
   > версию, поэтому `content_version` урока с PR-24 — версия его курса, раньше всегда 1); статья —
   > `knowledge_articles.version`. У теста, практикума и опроса версий нет, правка живая сразу —
   > «выше» значит «изменены после жалобы». Медиа закрывает только администратор. Проверка
   > выполняется в той же транзакции, что и публикация (`markContentChanged()` и правка вопроса,
   > поднявшая версию), и в момент отметки «Виправлено» — если исправление опубликовали раньше]
   > Ранее: правило без разбора по видам элементов
...
> [исправлено, PR-24: коды — в `DEFAULT_TEMPLATES` (`server/services/notifications.ts`) в виде
> `snake_case`, как у воронки PR-14; тексты и переменные — те же, что в реестре PR-37 (#115),
> одно определение на код. «in-app» без второго канала отправляется с `channel: 'inapp'`
> — только колокольчик. Троттлинг `merged` — расписанием «3-я и каждая 5-я» и пределом 3 в сутки на
> карточку (`mergedNotifyDue()`). «{{points}} балів» в `content_issue_fixed` не пишется: книга
> `points_ledger` появилась (#114, D-069), но события `content_issue` в перечне `points_event` нет, а
> новое значение перечня — через обсуждение (CLAUDE.md п. 13), §7.12. `content_issue_overdue` не
> отправляется — SLA-скан (§7.6, §11) в PR-24 не входит. Адресаты ответа — заявители, кроме уволенных (§12);
> адресаты работы (ответственный, администратор, руководитель) — только действующие] Ранее: без
> этих уточнений
...
> [исправлено, PR-24: (1) `PATCH /content-issues/:id` не принимает `severity`: её «считает система,
> не человек» (§3.1, §7.4); принимает ещё `internalNote` — «Внутрішня нотатка» формы §6.2.
> (2) Очередь листается ключевым курсором (`KEYSETS.contentIssues`, docs/04 §4.1:
> `reports_count desc, trusted desc, last_reported_at desc, id desc`), размер страницы
> `limit` — 10/25/50/100 (§5.3); в ответе `total` по фильтрам и `nextCursor`.
> (3) `rescore` принимает `mode: recalc|void|skip`; без `mode` режим берётся из резолюции.
> (4) Правила маршрутизации развёрнуты в четыре пути `/settings/content-issue-routing-rules`
> (GET — `triage`, POST/PATCH/DELETE — `assign`). (5) `GET /reports/content-quality` принимает
> `groupBy=element|course`, `drillKey` (детализация строки заявителями единым каркасом) и
> `format=xlsx`. (6) `Idempotency-Key` в репозитории не реализован нигде — долг вне пакета]
> Ранее: без этих уточнений
...
> [исправлено, PR-24: сделаны `content_issue.reassign_scan` (ежедневно, 03:50 по Киеву) и
> закрытие по публикации — не отдельной задачей, а в транзакции самой публикации (см. §7.9);
> `content_issue.rescore` — синхронно из запроса партиями по 200; `notify_reporters` — в момент
> закрытия. `sla_scan`, `content_link_check`, `content_media_check`, `content_quality.refresh`
> (отчёт считается напрямую) и `content_reporter.stats_refresh` — следующими PR] Ранее: все девять
> задач без отметки
...
**Г-36.7. Заголовок карточки** `[решение, PR-23]`. В §3.2 `title` — `not null`, 3–200 символов, но формы
с этим полем в §6.1 нет и быть не должно: «минимум трения» (§1) означает, что человек не придумывает
заголовок дефекта. Значит, заголовок обязан считаться. `[решение]` Сервер берёт его из самого материала:
название урока, ресурса, теста, практикума, опроса или статьи; у вопроса — первые 200 символов текста
без разметки; если материал названия не дал (медиа без имени), подставляется код типа проблемы. Тип
проблемы отдельной колонкой уже есть, поэтому дублировать его в заголовке незачем. Чем проверяется:
`tests/integration/v2-content-issues.spec.ts`, критерий 1 — карточка создана без единого введённого
символа, а `title` не пуст.

**Г-36.8. Когда карточка получает `affects_scoring`** `[решение, PR-23]`. §7.7 (в) говорит «карточка сразу
получает `affects_scoring = true` и `rescore_state = needed`», не уточняя, зависит ли это от типа проблемы.
`[решение]` Флаг ставится по **месту подачи**, а не по типу: любая жалоба на вопрос, поданная во время
попытки, помечает карточку как влияющую на баллы. Обоснование: тип выбирает человек в стрессе посреди теста,
и «Незрозуміло пояснено» на вопросе с неверным ключом — обычный случай; занижать по типу значит тихо терять
самый ценный сигнал, ради которого модуль и существует. Ложноположительный флаг стоит одного решения
администратора (`skipped` с комментарием, §7.8), ложноотрицательный — несправедливого балла.
Чем проверяется: тот же тест, критерий 3.

**Г-36.9. Журнал карточки заводится вместе с подачей** `[решение, PR-23]`. План (`45` PR-24) относил
`content_issue_events` к разбору, но критерий приёмки 2 требует событие `merged` в момент склейки, а §7.7 (б) —
запись компенсации времени попытки. `[решение]` Таблица создана миграцией `0067` вместе с подачей; PR-24
добавляет к ней переходы статусов, назначения и пересчёты. Чем проверяется: `36` §13 критерий 2.

**Г-36.10. Владелец категории курса** `[решение, PR-24]`. §7.5 (в) и критерий приёмки 8 называют
«владельца категории курса», но ни `course_categories` базового ТЗ (`docs/24` §3.7.1 — свободный
справочник с порядком), ни DDL §3.2, ни `40` такого поля не знают. Правило маршрутизации с
`category_id` его не заменяет: шаг (а) идёт раньше авторов, а «владелец» — после них. `[решение]`
Необязательная колонка `course_categories.owner_id` (миграция `0077`), задаётся через
`PATCH /course-categories/:id` (`ownerId`); категория без владельца шаг (в) пропускает. Чем
проверяется: `tests/integration/v2-content-triage.spec.ts`, критерий 8.

**Г-36.11. Назначение правилом не берёт в работу** `[решение, PR-24]`. §4 допускает прочтение «назначен
правилом → `in_progress`». Тогда вкладка «Нові» пустовала бы всегда (шаг (д) находит адресата почти
всегда), а заявитель получал бы «прийняли в роботу» раньше, чем человек карточку открыл. `[решение]`
Маршрутизация ставит ответственного и событие `assigned` от системы; «Взяти в роботу» — действие
человека, оно и шлёт `content_issue_accepted`. Чем проверяется: тот же тест, раздел маршрутизации.

**Г-36.12. Одна логика пересчёта — параметры, а не второй код** `[решение, PR-24]`. Условие выхода
плана: пересчёт по жалобе и по тесту — одна логика, две точки входа. При этом правила применения у
них разные: «Перерахувати» теста снимает зачёт и отзывает сертификат (D-013), пересчёт по жалобе —
«только улучшение» (§7.8). `[решение]` Подсчёт один (`planRecalc()`), запись одна
(`recalculateAttempt()`); различаются параметры — какие вопросы берут текущий ключ, какие исключены
из знаменателя, политика применения. Предпросмотр считает той же функцией без записи. Чем
проверяется: тест «одна логика, две точки входа» — «Перерахувати» теста даёт попытке ровно тот балл,
который пересчёт по жалобе показал в списке «с ухудшением».

**Г-36.13. Кому отвечать и кому поручать** `[решение, PR-24]`. §12 говорит «заявитель уволен —
уведомление не отправляется», §7.5 — «авторы неактивны (уволены, заблокированы)». Это два разных
вопроса. `[решение]` Поручают только действующим (`status = 'active'`, не заблокирован); отвечают
всем, кроме уволенных (`status <> 'archived'`) — приглашённый, ещё не входивший, свой ответ получит.
Чем проверяется: тот же тест (заявитель со статусом «приглашён» получает `content_issue_rejected`).
