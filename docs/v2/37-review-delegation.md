...
> [уточнено, PR-22: скоупы норм и отчёта §9.3 — решение Р-22.5, `46-progress.md`] Норму элемента
> и обезличенный отчёт §9.3 открывает тот, кому строка «Видеть «Розрахунковий час» и отклонение
> план/факт» даёт галочку: наставник и руководитель — по `time.metrics.view` (агрегаты по людям
> своей области), автор и администратор — по `course.edit` (весь тенант). Менять норму — только
> `course.edit`: норма — свойство материала. `time.metrics.view` автору по-прежнему не выдаётся —
> он открывает поимённое `GET /learning/time/totals`, а автору положен только агрегат.
...
(развязка цикла `40` §5 0022). **Поставлены** миграцией `0080_v2_review_delegation` под
именами В-13 (`rqi_delegation_id_fk`, `rqi_assigned_by_rule_id_fk`, оба `set null`).

> [исправлено, PR-19: состояний в §4 пять, а `check` таблицы PR-18 знал три] Ранее:
> `rqi_status_chk check (status in ('waiting','in_review','done'))`. Теперь
> `waiting | in_review | delegated | escalated | done` — ровно состояния §4; перечисление
> `review_queue_status` в `docs/02` расширено тем же PR (Р-19.1).
>
> [исправлено, PR-19: с маршрутизацией и делегированием «кто отвечает» и «у кого открыта
> карточка» перестали совпадать] Ранее: захват карточки жил в `assigned_reviewer_id` /
> `assigned_at`. Добавлены `claimed_by`, `claimed_at` — захват (30 минут, `13` §4.2);
> `assigned_reviewer_id` / `assigned_at` — только ответственность: правило, делегирование,
> переназначение, переброс. «Пропустити» снимает захват, а не назначение; эскалированный
> руководитель открывает работу, не отнимая её у назначенного. Прямой аналог зеркала
> `workshop_submissions.reviewer_id` / `claimed_at` (В-2), которое снимет PR-20 (Р-19.2).
...
> [исправлено, PR-19: уникальность «одно активное на элемент» несовместима с цепочкой §7.3 и
> возвратом §7.5] Ранее: `uq_review_delegations_active on (tenant_id, queue_item_id)`. При
> A→B→C звено A→B обязано оставаться в силе: истечение B→C возвращает работу B «с глубиной на
> единицу меньше» (§7.5), и A→B продолжает идти свой срок. Реализовано
> `unique (tenant_id, queue_item_id, depth) where state = 'active'` — два одновременных звена
> одного уровня (A→B и A→D) по-прежнему невозможны. `review_queue_items.delegation_id` смотрит
> на самое глубокое активное звено. Плюс `request_context` (правило 14 — журнал передач) и
> `rd_revoke_reason_chk` (≤ 500, как в форме §6.1) (Р-19.3).
...
> [исправлено, PR-19: `40` §7.2 — частичный индекс не обслуживает экран, где видны и
> выключенные правила] Добавлен полный `idx_review_routing_rules_tenant_all (tenant_id,
> priority)`; частичный остаётся для горячего пути. Плюс `rrr_task_types_chk` (виды работ —
> только из `review_task_type`) и `rrr_name_chk` (1–200).
...
> [дополнено, PR-33, `41` §8.3.4] `reviewer_absences` — отсутствие **проверяющего** (заместитель,
> переброс очереди); отпуск и больничный человека для карточки и планирования обучения — другая
> сущность, `absence_records` (`docs/v2/38` §3.6). Автоматической связи между ними нет: запись
> отпуска в карточке очередь проверки не перебрасывает — открытый вопрос `41` §8.3.4, записан в
> `46-progress.md` (PR-33).

> [исправлено, PR-19] Три уточнения DDL ниже, каждое с причиной (`46-progress.md` Р-19.4):
> `review_sla_events.details jsonb` — «`assigned` с пометкой перегрузки» (§7.17) и адресат
> эскалации (руководитель, уровень выше по дереву — PR-31 — или администратор) требуют места, которого в DDL не было;
> `review_sla_events.request_context` — правило 14; `reviewer_capacity.task_types` проверяется
> по `review_task_type`. `rr_cursor` — счётчик работ, полученных человеком по кругу (§7.17).
...
«Орієнтовний час» материала (`resources.estimated_minutes`, `11` §3.1) остаётся полем автора и синхронизируется в `author_seconds` при сохранении материала; авторитетной для очереди и отчётов становится эта таблица.

> [исправлено, PR-22: поля `lessons.estimated_minutes` нет ни в `11` §3.1, ни в схеме — `11` §3.1
> описывает материал `resources`, и поле автора живёт там] Ранее: «`lessons.estimated_minutes`
> (`11` §3.1) остаётся полем автора…». Урок — ссылка на материал, у одного материала бывает
> несколько уроков в разных курсах и версиях курса. Поэтому сохранение материала с новым
> «Орієнтовним часом» становится нормой (`source = 'author'`) каждого урока с этим материалом,
> у которого строка нормы уже есть, а урок без строки берёт это число нормой по умолчанию;
> очищенное поле возвращает к авторасчёту только нормы, взятые из него же (Р-22.2).
>
> [исправлено, PR-22: DDL не давал выполнить §6.3 и §7.14 — решения Р-22.1…Р-22.3] Ранее: DDL
> выше без перечисленного ниже. Миграция `0085_v2_time_norms` добавляет общие `created_at`/
> `updated_at` (`docs/02`, преамбула); `deviation_flag` по умолчанию `no_data`, а не `none` — у
> новой нормы выборки нет, и по §7.14 это `no_data` (`none` утверждал бы «измерили, отклонения
> нет»); `updated_by` — `on delete set null`; ограничение `ctn_author_source_chk` (у `author` и
> `observed` число обязано быть) и `ctn_observed_chk` (счётчики неотрицательны, p25 ≤ медиана
> ≤ p75). **`observed` хранит число в `author_seconds`**: «Застосувати» замораживает медиану в
> момент нажатия, иначе еженедельный пересчёт `observed_seconds` молча двигал бы норму, а §6.3
> это запрещает. `observed_seconds`, p25, p75 — статистика факта, не норма; при выборке меньше 10
> они не хранятся вовсе (Р-22.4).
...
> [исправлено, PR-22: экран норм и отчёт §9.3 — одна страница, решение Р-22.9] Ранее: отдельный
> экран `/content/time-norms`. Строки и колонки экрана — подмножество отчёта §9.3 (тот добавляет
> «Трек» и «Частку недостовірних вимірів»), две страницы были бы двумя источниками одних и тех
> же чисел. Сделано одной страницей `/admin/reports/time-plan-fact` в разделе «Звіти»: над
> таблицей — эта жёлтая плашка, фильтр по отклонению — тот же, а тот, кто правит контент
> (`course.edit`), открывает форму §6.3 прямо в строке («Змінити норму»).
...
| Термін для делегата | дата-время | ✓ | ≥ +12 год, ≤ `sla_due_at` элемента; у второго звена ещё и ≤ срока первого | «Термін не може бути пізнішим за термін перевірки» |
| Повідомити делегата | переключатель | — | вкл. по умолчанию | — |

> [исправлено, PR-19: когда истечёт A→B, работа вернётся к A вместе со всем, что B передал
> дальше, — срок, выданный C позже этого момента, был бы обманом] Ранее: верхняя граница — только
> `sla_due_at` элемента.
...
> [уточнено, PR-22] «За фактом» и «Застосувати» — одно действие: медиана округляется вверх до
> целой минуты (форма ведёт норму в минутах) и замораживается в норме (§3.5, Р-22.2); до 20
> прохождений радио-кнопка «За фактом» неактивна, запрос — `422 norm.sample_too_small`. При «Авто»
> форма показывает авторасчёт по объёму или «Для цього елемента авторозрахунку немає — вкажіть
> час вручну» (практикум, шаг траектории, документ). Сохранение сразу пересчитывает флаг по уже
> посчитанной медиане; уведомление автору шлёт только еженедельный пересчёт (Р-22.6). Правка
> нормы пишется в `audit_log` (`time_norm.update`, `time_norm.apply_observed`,
> `time_norm.sync_material`).
...
2. **Кто кому может делегировать.** Делегирует тот, кому работа назначена (`assigned_reviewer_id`), при наличии `review.delegate`. Делегат обязан одновременно: (а) иметь `review.grade`; (б) иметь область, покрывающую `location_id` элемента (`01-roles.md`) — **нельзя делегировать тому, кто не имеет доступа к этой точке**; (в) при `subject_kind='candidate'` иметь право на назначенную проверку кандидата (`28` §2); (г) не быть самим проверяемым (п. 7); (д) иметь `accepts_delegation = true`. Нарушение (а)–(г) — `422 review.delegate_target_forbidden`, (д) — `422 review.delegate_target_declines`. Список в форме отфильтрован заранее: недоступные не показываются вовсе.
   `[решение PR-19]` Работу из **общего пула** (правил нет или `manual` — `assigned_reviewer_id is null`) делегирует любой, кто вправе проверить её сам: без правил она назначена каждому проверяющему точки (`13` §7.1), и без этого в тенанте без правил делегировать было бы нечего. Чужую назначенную работу передаёт только `review.delegate.any` — звено пишется от имени текущего ответственного, нажавший виден в `audit_log`. Точка кандидата, у которого размещения нет (`28` §2), — точка его вакансии: без неё работа кандидата не раскрывала бы области для (б) (Р-19.6).
...
5. **Делегат не уложился.** В `due_at` делегирования работа **автоматически возвращается** делегировавшему: `state = 'revoked_sla'`, `assigned_reviewer_id = from_user_id`, `delegation_depth − 1`, обоим уходит `review_delegation_expired`, в `review_sla_events` пишется `delegation_expired`. Исходный `sla_due_at` элемента **не сдвигается** — иначе делегирование стало бы способом бесплатно продлить срок.
   `[решение PR-19]` Открытую прямо сейчас карточку (захват моложе 30 минут) возврат не вырывает из рук: он случится на следующем прогоне `review.delegation_expire`, когда захват протухнет, — тот же принцип, что у переброса очереди отсутствующего (п. 18). В цепочке A→B→C истекает только своё звено: B→C вернёт работу B с глубиной 1, A→B останется в силе.
6. **Отзыв.** Делегировавший отзывает своё делегирование, пока элемент в `waiting` или `delegated`. Как только делегат перевёл в `in_review` — `409 review.delegation_in_progress`; руководитель с `review.delegate.any` может отозвать и тогда, с обязательной причиной. Любой отзыв возвращает `assigned_reviewer_id` делегировавшему и пишется в `audit_log`. Отзыв звена закрывает и всё, что ниже него: A, отозвав A→B при активном B→C, возвращает работу себе, звено B→C закрывается тем же состоянием.
...
13. **«Розрахунковий час» — откуда берётся** `[решение]`. Три источника (`content_time_norms.source`):
    - `author` — автор ввёл вручную (форма §6.3 или «Орієнтовний час» материала `resources.estimated_minutes`, `11` §3.1). Приоритетный.
    ...
    - `observed` — медиана фактического `content_seconds + attempt_seconds` по завершённым прохождениям при `observed_sample ≥ 20`, **никогда не применяется автоматически**: система предлагает, автор нажимает «Застосувати» (§6.3).

    > [исправлено, PR-22: ссылки вели мимо — поля `lessons.estimated_minutes` нет (§3.5), §6.4 в
    > документе нет, форма «Застосувати» — §6.3] Ранее: «(`lessons.estimated_minutes`, `11` §3.1)»
    > и «(§6.4)».
    >
    > [уточнено, PR-22: чего авторасчёт не видит — решение Р-22.7] Число страниц документа нигде
    > не хранится (у `media_assets` нет такой колонки) — авторасчёта у документа нет, только
    > `author`. Текст урока — `plain_text` той версии материала, что закреплена публикацией курса.
    > У теста со случайной выборкой вопросов их число — сумма `count` правил выборки, а какие из
    > них развёрнутые, заранее не известно: развёрнутых — 0. У урока-теста и урока-практикума
    > авторасчёта нет: время их выполнения меряется на самом тесте и практикуме. Шаг траектории —
    > только `author`, как практикум.
...
> [уточнено, PR-35] Правило (б) победило в споре с `docs/v2/38` §7.3 («`E` не начисляется быстрее
> `Плановий час × 0.5`»): індекс залученості норму времени не читает вовсе (`38` §15, Г-38.9).
> «Плановий час» трека на карточке человека (`38` §5.1) берётся из модуля норм третьим именованным
> входом — `versionPlannedSeconds()`, только для показа рядом с «Часом проходження»; сквозная
> проверка 13 знает этот вход поимённо.

    > [уточнено, PR-22: что такое «прохождение» и чем гарантировано (б) — решения Р-22.4, Р-22.10]
    > Прохождение — строка витрины `learning_time_totals` («человек × элемент × запись на курс»,
    > время всех попыток и устройств в ней уже сложено свёрткой), у которой есть отметка
    > завершения самого элемента: урок зачтён; попытка теста отправлена (`submitted`, `review`,
    > `passed`, `failed` — не просроченная и не аннулированная); сдача практикума отправлена;
    > шаг траектории пройден. Люди — единым каркасом отчётов (`frameWhere()`): только сотрудники
    > (П-16.1), архивированные исключены; недостоверные (`unreliable`) — в долю «недостовірних
    > вимірів», а не в выборку. Медиана, p25 и p75 **не хранятся и не показываются при выборке
    > меньше 10**: на двух-трёх прохождениях «обезличенный» процентиль — это время конкретного
    > человека, чьё имя известно из состава курса. Пункт (б) — не только обещание: норма и флаг
    > живут в одном модуле (`server/services/timeNorms.ts`), наружу из него выходят ровно два
    > имени — снимок нормы писателям очереди и синхронизация «Орієнтовного часу» материала, — и
    > это сторожит тринадцатая сквозная проверка `scripts/v2-crosschecks.sh`; тест критерия 11
    > сравнивает все таблицы с баллами тенанта до и после всего, что делает модуль.
...
17. **Круговое распределение.** Кандидаты сортируются по `users.id`, указатель `reviewer_capacity.rr_cursor` продвигается на одного за назначение и сохраняется в той же транзакции (`select … for update`).
    > [исправлено, PR-19: у одного `rr_cursor` на человека нет «указателя на следующего» — есть счётчик] Реализовано так: `rr_cursor` — сколько работ человек получил по кругу; следующим идёт тот, у кого он меньше, при равенстве — первый по `users.id`. На постоянном составе это ровно круг A→B→C→A; отсутствующий выпадает без «долга», новичок входит со счётчиком, равным текущему минимуму. Пропускаются отсутствующие (п. 18), достигшие `max_open_items` и исключённые по конфликту интересов. Если пропущены все — берётся первый по списку с игнорированием лимита, в `review_sla_events` пишется `assigned` с пометкой перегрузки: работа не может остаться неназначенной из-за лимитов.
...
19. **SLA и эскалация.** Срок — `sla_hours` элемента (снимок, по умолчанию 48), отсчёт от `assigned_at`, при отсутствии назначения — от `submitted_at`.
    > [исправлено, PR-19: «от `assigned_at`» противоречит условию «делегирование срок не продлевает» — делегирование и переназначение меняют `assigned_at`] Ранее: «отсчёт от `assigned_at`». Срок `sla_due_at` ставится один раз, при постановке, от `submitted_at` (правило распределения назначает в той же транзакции — это тот же момент; его `sla_hours_override` — снимок на момент постановки) и дальше **не пересчитывается** ни делегированием, ни переназначением, ни перебросом, ни ежедневной перебалансировкой. Пороги считаются от окна `sla_hours`, конец которого — `sla_due_at`. Руководитель области — `resolveManager()` проверяемого (П-16.4); если он сам проверяющий — уровнем выше по дереву, выше некуда — администратор тенанта (фолбэк PR-19; адресат и источник пишутся в `review_sla_events.details`).
    > [исправлено, реализация PR-31: «уровнем выше по дереву» теперь есть у любого руководителя, а не только у руководителя из дерева при включённом `org_structure_is_source_of_truth`. Цепочка — `escalationChainOf()` (`server/services/orgManager.ts`): первое звено — `resolveManager()`, дальше — держатели руководящих точек над основным узлом этого руководителя, ближайший первым, вакантные пропускаются («до ближайшего держателя»); руководителя в дереве нет — подъём от узла самого проверяемого. Уровни выше берутся из дерева и при выключенном флаге: флаг решает спор двух источников о **прямом** руководителе, а на вопрос «кто над руководителем точки» у `locations.manager_id` ответа нет вовсе. Администратор тенанта — только когда выше в дереве никого; `details.target` — `manager` \| `tree` \| `admin`. Та же цепочка — у запасного проверяющего распределения (§7.16)] Ранее: после руководителя точки или роли в области следующим шагом сразу шёл администратор тенанта — «фолбэк PR-19». Делегирование срок не продлевает (п. 5). Пороги: **50 %** — уведомление проверяющему `review_sla_warning`, `sla_warned_at`; **100 %** — `sla_breached_at`, строка коралловая, руководителю `review_sla_breach`; **150 %** — эскалация: `status = 'escalated'`, `escalated_to_id` = руководитель области, работа появляется в его «Мої» **дополнительно** к текущему проверяющему (единственный случай, когда работу видят двое — чтобы эскалация не выглядела как отъём). Просрочкой в отчётах считается факт `sla_breached_at is not null` на момент решения, а не текущее состояние.
...
> [уточнено, PR-22: когда и кому уходит `content_time_deviation` — решения Р-22.6, Р-22.8] Шлёт
> только еженедельный `time.norms_recalc` и только **при входе** элемента в отклонение на выборке
> ≥ 20: флаг появился, сменил сторону или выборка доросла до 20 при уже стоящем флаге. Держится
> неделями — повторно не уведомляет; вернулся в норму и снова ушёл — уведомляет заново. Правка
> нормы автором не уведомляет — он видит флаг на экране сам. Адресаты — **действующие** авторы
> элемента (материала урока, теста, практикума; у урока без авторов — создатель курса): уволенный
> через офбординг и заблокированный — не адресаты, то же условие, что у маршрутизации жалоб
> (`36` §7.5 б, `ACTIVE_EMPLOYEES_ONLY`). Если действующих авторов не осталось — владелец
> категории курса (`36` §7.5 в): сигнал о материале, который никто не ведёт, не должен
> пропадать. `{fact}` и `{plan}` — целые минуты, отформатированные локалью адресата
> (`users.locale ?? tenants.locale ?? 'uk'`); кнопка «Відкрити» ведёт в отчёт §9.3 на этом элементе.
...
**9.3 «План і факт часу»** (по контенту, обезличенный). Колонки: Елемент · Тип · Трек · Розрахунковий час · Джерело норми · Медіана факту · p25 · p75 · Вибірка · Відхилення · Частка недостовірних вимірів. Фильтры: тип элемента, трек, отклонение, минимальный размер выборки. Шапка — жёлтая плашка §5.3.

> [исправлено, PR-22: §5.5 в документе нет, плашка «Це показник якості матеріалу…» — у экрана
> норм §5.3] Ранее: «Шапка — жёлтая плашка §5.5».
>
> [уточнено, PR-22: как устроен отчёт — решения Р-22.4, Р-22.9] `GET /reports/time-plan-fact`,
> экран `/admin/reports/time-plan-fact` (он же экран норм §5.3). Считается сразу при запросе
> (`docs/22` §7.7 «Розраховано зараз») **тем же расчётом, что и норма** (§7.14): отчёт по всему
> тенанту с фильтрами по умолчанию даёт ровно те флаги, что пересчёт положил в
> `content_time_norms`. Люди — единым каркасом отчётов (`docs/22` §13.3): только сотрудники,
> архивированные исключены (переключатель — `includeArchived`), руководитель и наставник видят
> агрегаты по людям своей области, фильтр «точка» только сужает. Сверх фильтров §9.3 — период
> по дате завершения прохождения (без периода — всё время, как у нормы). **Детализации «кто
> составил цифру» нет** (`docs/22` §3 п. 4 здесь не применяется): поимённое время — только
> `GET /learning/time/totals` по `time.metrics.view`. Медиана, p25, p75 — с 10 достоверных
> прохождений, доля недостоверных — с 10 прохождений всего, иначе «—». Каждый флаг
> `too_slow`/`too_fast` подписан «Сигнал якості матеріалу, не оцінка людини» (§7.14 а).
> Выгрузка xlsx — `report.export`, теми же строками, что на экране; в конструктор выгрузок отчёт
> регистрирует PR-38 (П-22).
...
|---|---|---|---|---|
| GET | `/review/queue` | `?tab=mine\|delegated_in\|delegated_out\|done&taskType&locationId&trackId&from&to&reviewerId&subjectKind&overdue&cursor&limit` | `{data:{items,total,cursor,counts}}` | `403 forbidden` |
| GET | `/review/items/:id` | — | `{data:{item, subject, contactsHidden, delegations, conflict, can, work}}` — **без `phone`, `email`, `resume_asset_id`** (сквозная проверка 20) | `404` |
| GET | `/review/items/:id/delegate-targets` | — | `{data:[{id, fullName, open, max, overloaded}]}` — список формы §6.1, отфильтрованный заранее | `404` |
| PATCH | `/review/capacity/:userId` | `{maxOpenItems?, dailyTarget?, acceptsDelegation?, taskTypes?}` | `{data:{ok:true}}` | `403 forbidden` |
...
> [уточнено, PR-22: нормы времени и отчёт §9.3] Вход PUT — `{source: 'author', authorSeconds}`
> | `{source: 'auto'}` | `{source: 'observed'}` (последнее — то же, что `apply-observed`: при
> выборке меньше 20 — `422 norm.sample_too_small`). Ответ GET/PUT/`apply-observed` — норма:
> `{subjectType, subjectId, title, persisted, source, plannedSeconds, authorSeconds, autoSeconds,
> observedSeconds, observedP25, observedP75, observedSample, deviationFlag, canApplyObserved,
> recalculatedAt, updatedAt}`; у элемента без строки нормы (`persisted: false`) — норма по
> умолчанию: «Орієнтовний час» материала или авторасчёт. Чужой тенант, удалённый и
> несуществующий элемент — `404` (CLAUDE.md п. 15). Скоупы — §2: читать — `time.metrics.view` или
> `course.edit`, менять — `course.edit`; правка — в `audit_log`. `/reports/time-plan-fact` — ответ
> `{rows, summary, computedAt}`, строка — элемент без людей, `format=xlsx` — `report.export`.
...
| `time.norms_recalc` | еженедельно, ночь воскресенья | пересчёт `observed_seconds`, p25, p75, `deviation_flag`, уведомление автору; воскресенье 02:30 по Киеву (PR-22), строка нормы появляется у каждого элемента с хоть одним завершённым измеренным прохождением |
...
- **Эскалация на руководителя, который сам и есть проверяющий** → уходит на уровень выше по `org_nodes` (`32`) — до ближайшего держателя руководящей точки, вакантные узлы пропускаются; выше некуда — администратору тенанта. (PR-31: подъём по дереву есть и у руководителя точки, не только у руководителя из дерева.)
...
**Г-37.15. Когда норма «застосована» и что с ней делает пересчёт** `[решение, PR-22]`. §6.3 требует «молча система норму не меняет никогда», а `observed_seconds` пересчитывается еженедельно. `[решение]` Р-22.2: «Застосувати» замораживает медиану (вверх до целой минуты) в `author_seconds` с `source = 'observed'`; пересчёт обновляет статистику факта, но не норму. «Орієнтовний час» материала при сохранении становится нормой его уроков — это явное действие автора, и последнее такое действие выигрывает. Чем проверяется: `tests/integration/v2-time-norms.spec.ts` («Застосувати» замораживает норму); в эксплуатации — доля норм `observed`, чья медиана за квартал ушла от нормы больше чем вдвое: если заметна, стоит показывать автору «медіана змінилась з моменту застосування».

**Г-37.16. Показывать ли медиану маленькой выборки** `[решение, PR-22]`. §2 обещает автору агрегат без имён, §7.14 не даёт флага при выборке меньше 10, но про показ медианы молчит. `[решение]` Р-22.4: медиана, p25 и p75 не хранятся и не показываются при выборке меньше 10 — на двух-трёх прохождениях процентиль и есть время конкретного человека, а состав курса автору известен. Порог — тот же, что у флага: одна граница «данных достаточно». Чем проверяется: число строк отчёта §9.3 с «Немає даних» при выборке 5–9 за первый квартал; если таких много и авторы просят цифры — порог снижается до 5 с подписью «мала вибірка».

**Г-37.17. Когда уведомлять автора об отклонении** `[решение, PR-22]`. §8: «флаг `too_slow`/`too_fast` при выборке ≥ 20» — не сказано, уведомлять ли каждую неделю, пока флаг держится. `[решение]` Р-22.6: только при входе в отклонение (флаг появился, сменил сторону или выборка доросла до 20 при стоящем флаге); еженедельное повторение одного и того же приучило бы игнорировать сигнал. Чем проверяется: доля уведомлений `content_time_deviation`, после которых норма или материал поменялись в течение 30 дней; если ниже 10 % — сигнал не работает, и нужен дайджест вместо поштучных.

**Г-37.18. Кому сигнал, если авторы ушли** `[решение, PR-22]`. §8 называет только «автора материала». `[решение]` Р-22.8: действующие авторы; уволенный через офбординг и заблокированный — не адресаты (как у жалоб, `36` §7.5 б); не осталось никого — владелец категории курса (`36` §7.5 в). Сигнал о материале, который никто не ведёт, не должен пропадать, а владелец категории — тот, кто уже получает жалобы на такой материал. Чем проверяется: `v2-time-norms.spec.ts` (все авторы ушли — уведомление владельцу категории); в эксплуатации — число элементов с отклонением без единого адресата (курс без категории или категория без владельца).
