...
**Сеанс измерения** (`learning_time_sessions.closed_reason`): открыт → закрыт одной из семи причин §3.6. Открытых сеансов у человека по одной паре `(subject, kind)` не больше одного; попытка открыть второй закрывает первый с `navigated_away`.
...
| `time.rollup` | каждые 10 минут | свёртка сеансов в `learning_time_totals`, `lesson_progress`, `attempts`, `workshop_submissions` |
