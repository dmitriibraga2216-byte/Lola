...
> [исправлено, реализация PR-35] Частичный индекс — `where kind = 'employee'`, **без** `and status =
> 'active'` (Р-35.1): вкладка «Активні» списка — это `invited` + `active`, у архивированных
> фиксируется последнее значение (§7.2), а индекс с предикатом `status = 'active'` планировщик не
> взял бы ни для одной вкладки. `rating_pct` получил `check` 0…130 (`users_rating_pct_chk`) — тот же
> диапазон, что у снимка. Колонка — **індекс навчальної залученості**, а не баллы рейтинга
> (`points_ledger`, «Поточний рейтинг», `docs/33` D-069); в подписях интерфейса слово «рейтинг»
> осталось за баллами. Ранее: «`… where kind = 'employee' and status = 'active'`».
...
> [исправлено, реализация PR-35] Отдельного `idx_person_rating_snapshots_tenant (tenant_id, user_id,
> calc_date desc)` нет: уникальный ключ `(tenant_id, user_id, calc_date)` — тот же btree, читаемый и в
> обратном порядке (тот же довод, что у `user_activity_daily`, PR-34). Сверх DDL: частичный
...
> [уточнено, реализация PR-35, П-16.2] Карточка — одной страницей на вкладке «Профіль» в порядке
> этого параграфа; вкладка «Активність» — журнал действий, прежние вкладки «Нотатки», «Документи»,
> «Відсутності» стали свёрнутыми секциями (ссылка `?tab=…` открывает секцию). Шапка — текущий этап
> человека «{Етап}, з {дата}» (`33` §5.3, при `lifecycle.view`) и чип индекса. Треки
> (`GET /people/:id/tracks`) — все группы по возможностям этапа через `stageCan()`: этап без
> `progress` — плоский список «назва · Призначено», без процента и срока (`33` §13 к. 2); пустая
> строка — только у включённого этапа, который назначается сотруднику (`applies_to_employee`);
> курс рекрутинга, пройденный ещё кандидатом, остаётся в своём этапе. Отступления: **обложка** —
> инициал курса на цвете этапа (подписанные ссылки на обложки карточке не нужны, Р-35.7);
> **«Пройдено модулів X/Y»** — обязательные уроки версии записи (`required_done`/`required_total`,
> §7.15) — те же счётчики, из которых сервер считает процент, иначе «8/10» при 100 %;
> **«Плановий час»** — сумма норм элементов версии записи (`versionPlannedSeconds()`, модуль норм
> PR-22: уроки, тесты и практикумы в них), **«Час проходження»** — «контент + випробування» записи из
> витрины учёта времени PR-21; обе — только тому, кому видно время человека (сам, `time.metrics.view`
> в области его точки, `37` §2), остальным строк нет. Светофор считает сервер; у завершённой записи
> срок — датой без тревоги.
...
> [уточнено, реализация PR-35] Цифра — только носителю `person.rating.view_others` и только по
> людям его области (руководитель — своих точек): у остальных строк ячейка пустая, а фильтр и
> сортировка считают их «не рассчитанными» — иначе порядок строк выдал бы сравнение людей вне
> области (§7.3). Сортировка — **только по убыванию** (Р-35.3): «антитопа» первой страницей нет, места
> в списке не показываются; `null` — в конце. Цвет чипа — бирюза («результат» бренд-бука) для любого
> значения, без светофора по величине: красный на «45 %» и есть та наказательная подача, от которой
> §7.3 защищает. Фильтры «Індекс від, %» / «Індекс нижче, %» (`?ratingGte=` / `?ratingLt=`).
...
> [уточнено, реализация PR-35] Экран один на два адреса: свой индекс — `/learn/profile/rating`
> (строка «Індекс залученості» под плитками профиля), чужой — `/admin/people/:id/rating` (чип в шапке
> карточки). Заголовок чужого — «Звідки взявся відсоток: {ПІБ}», текст досрочности — в третьем лице.
> Сверх описанного: «Динаміка за 12 місяців» (§7.3 — «своя динамика», последнее значение месяца);
> «Дані оновлюються», если значение старше 48 часов или ещё не считалось; кнопка «Перерахувати»
> (`POST /people/:id/rating/recalc`, раз в час); строка «Це не бали рейтингу…». Список назначений
> окна — `<details>` с вкладом «+{wᵢ × cᵢ} з {wᵢ}» у каждой записи.
...
> [уточнено, реализация PR-35] Формула — `computeEngagementIndex()` (`shared/domain/engagementIndex.ts`,
> unit-тест `engagement-index.spec.ts`), пересчёт — `server/services/engagementIndex.ts`. Доопределено:
> (1) **статусы** — в Lola `enrollment_status` (`docs/02`): `done` ↔ `completed`, «прострочено» —
> это `failed` с `expired_at`; заявка каталога `not_assigned` в окно не входит; (2) **окно** — плюс
> «все активные на дату расчёта», минус снятые (`cancelled_at`), ещё не открытые (`starts_at` в
> будущем — пройти их нельзя) и курсы этапа без `counts_in_rating` (`33` §3.3: «назначения этапа не
> влияют на рейтинг», критерий `33` §13 к. 4) — решение по этапу через `stageCan()`; та же
> возможность выключает и баллы (`docs/21` §17), поэтому её подпись в настройках этапов — «Бали й
> індекс»; (3) **`assigned_at`** — момент, с которого можно проходить: запись на курс, а если доступ
> открылся позже — `starts_at`; срок раньше этого момента в `E` не входит (деления на ноль нет);
> (4) **обязательность** — `assignments.is_mandatory` записи, самозапись — добровольная; (5) **`S` и
> `H`** — из суточного агрегата ленты `user_activity_daily` (дни с `level > 0`; `kinds.review_graded`
> + `kinds.content_issue_accepted`), он хранится бессрочно — число воспроизводится и после
> `activity.purge` (§7.2); (6) **округление** — каждое слагаемое до десятых, итог — сумма
> округлённых: экран складывает ровно то, что показывает. Не сделано `[решение]` (§15, Г-38.9,
> Г-38.10): порог «`E` не начисляется быстрее `Плановий час × 0.5`» (§7.3) и «менторское
> сопровождение» в `H`.
...
> [уточнено, реализация PR-35] Запрет (1) — по **фильтру**: `POST /people/bulk` принимает либо
> отмеченных (`ids`), либо «всіх за фільтром» (`filter` — те же условия, что у списка); фильтр, где
> кроме порога индекса нет ни одного сужающего условия (поиск, точка, посада, рівень, місто,
> підрозділ, роль, мітка, даты — вкладка статуса и «скрытые» не в счёт), при «Архівувати» — `422
> rating_only_filter_forbidden`. Отмеченные поимённо — решение по каждому человеку, а не условие
> (Р-35.5); экран всё равно гасит «Архівувати», пока список отфильтрован одним индексом. Массовой
> «Заблокувати» в списке нет — правило держит её код заранее (`RATING_GUARDED_BULK_ACTIONS`).
> Запрет (2) — галка «Включити індекс залученості» у «Експорт в Excel» с подтверждением,
> `?includeRating=1`. Запрет (3) — доски лидеров по индексу нет нигде; «Распределение по точке»
> (гистограмма без имён) PR-35 не делал — см. `46-progress.md`. Защита формулы «`E` не
> начисляется, если между `started_at` и `completed_at` прошло меньше `Плановий час × 0.5`» **не
> реализована** `[решение]` Р-35.2 — §15, Г-38.9.
...
> [уточнено, реализация PR-35] Рейтинговые пути: `GET /people/:id/rating` отдаёт `{person, self, state,
> total, base, bonuses: {early, streak, help}, breakdown, window, calcDate, updatedAt, stale, formula:
> {version, changedAt}, history}` — `state`: `ok` · `no_assignments` («Показник не розраховується…») ·
> `pending` (ещё не считался). Скоупа на входе нет — права решает сервис (свой — без скоупа); «нет
> скоупа» — `403 forbidden` (`44` В-16), кандидат и чужой тенант — `404`. `POST
> /people/:id/rating/recalc` считает **в запросе** и отвечает как `GET` `[решение]` Р-35.6 (ранее
> `{jobId}`: партия из одного человека — три запроса, очередь лишь откладывала бы ответ); сверх
> `429 recalc_too_often` — `409 person_archived` (уволенному значение зафиксировано, §7.2); чужой
> пересчёт — `person_rating.recalc` в `audit_log`. Новые пути: `GET /people/:id/tracks` (блок «Етап»,
> §5.1), фильтры `?ratingLt=&ratingGte=` и `?sort=rating` у `GET /people`, `{filter}` у `POST
> /people/bulk` с `422 rating_only_filter_forbidden` (§7.3), `?includeRating=1` у `GET
> /people/export`. Отчёт `/reports/rating` (§9 п. 5) — PR-38.
...
> [дополнено, PR-35] `rating.recalc` работает (pg-boss, 04:00 по Киеву, круг `runPerTenant`):
> сотрудники кроме уволенных, по возрастанию `id` партиями по 500, каждая партия — своя транзакция
> `withTenant()`. Повторный расчёт в тот же день перезаписывает снимок дня; «не рассчитан» снимает
> `is_current` и пишет `rating_pct = null`. Уведомление `rating_formula_changed` (§8) не заведено:
> версия формулы одна — появится вместе со второй.
...
**Г-38.9 — порог досрочности «`Плановий час × 0.5`» против правила «время не входит в балл»** `[решение, PR-35]`.
Документ противоречит соседнему: §7.3 требует не начислять `E`, если между `started_at` и `completed_at` прошло меньше
половины «Планового часу» (`37`), а `37` §7 п. 14 (б) запрещает времени участвовать «ни в одной формуле балла, зачёта,
рейтинга или начисления баллов — ни прямо, ни коэффициентом»; этот запрет уже в `main` держат сквозная проверка 13
и критерий `37` §13 к. 11 (PR-22). `[гипотеза]` Обе ветки защищают от одного — индекс как мерка человека: `38` — от
«сдал за минуту ради досрочности», `37` — от «медленный/быстрый» как оценки. `[решение]` Прав `37`: порог по норме —
это и есть отклонение факта от плана (`too_fast` там — при 0.4, здесь — 0.5), превращённое в минус человеку, хотя
отклонение — «сигнал качества материала, а не оценка людини»; норма бывает занижена автором, и тогда быстрый честный
человек терял бы бонус. Досрочность считается только по датам; от «пройти лёгкое ради процента» защищают вес
добровольного (1 против 2), потолок `H` в 20 действий и `S` по завершённым событиям. Сквозная проверка 13 закрепляет:
модуль норм не импортируется сервисом индекса (фикстура в `v2-crosschecks.spec.ts`), а карточка берёт «Плановий час»
только для показа — третьим именованным входом (`versionPlannedSeconds`). *Чем проверяется:* жалобы «пройшов за 10
хвилин, а досрочность повна» на курсах, где норма адекватна; если станет практикой — отдельное решение с владельцем
продукта, а не порог по норме.

**Г-38.10 — «завершённое менторское сопровождение новичка» в `H`** `[решение, PR-35]`. У действия нет события ленты:
закрытый список `user_activity_kind` (§7.9) его не содержит, а «сопровождение» в Lola — это наставник плана развития
(`docs/19` §3.4) или узел траектории «Призначити наставника» (`docs/17`), и ни у одного нет отметки «сопровождение
завершено». `[гипотеза]` Имелось в виду завершение наставником плана адаптации новичка. `[решение]` В `H` пока
только `review_graded` и `content_issue_accepted`; третье — новым значением перечисления (`docs/02`, CLAUDE.md п. 13)
и источником события, по обсуждению. *Чем проверяется:* вопрос владельцу продукта — что считать завершением
сопровождения.

**Г-38.11 — сортировка колонки «%» против «места в ранжированном списке»** `[решение, PR-35]`. §5.2 требует сортировку,
§7.3 запрещает место в списке, «топ» и «антитоп» и руководителю. `[гипотеза]` Запрет — о показе позиции («ви на 5-му
місці») и о витринах лучших/худших, а сортировка таблицы — инструмент носителя скоупа внутри его области (под неё
заведён частичный индекс §3.1). `[решение]` Сортировка есть, только по убыванию; позиции не показываются нигде; вне
области смотрящего строки сортируются как «не рассчитан» — порядок не выдаёт чужих цифр. *Чем проверяется:* нет ли
у тенантов запроса «покажіть найгірших» — если есть, это сигнал, что индекс начали использовать для наказания (§7.3).
