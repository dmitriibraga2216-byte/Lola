...
| `interview_scenarios` | Сценарий авто-собеседования поверх `quizzes` с `mode='interview'` | Т |
...
Нумерация продолжает стадию 0. **Номер `0003` уже занят** файлом
`lola/drizzle/sql/0003_indexes.sql`, поэтому пакет начинается с `0004`. Всего 25 файлов.
...
Расширения, которых нет в `0001_foundation.sql` (там только `pgcrypto` и `pg_trgm`).

- `create extension if not exists ltree` — нужен `org_nodes.path` (`32`).
- `create extension if not exists vector` — нужен `library_modules.embedding vector(768)`
  и индекс `hnsw` (`31`).

Без этого файла миграции 0011 и 0020 не выполнятся. Пропуск этого шага — самый
вероятный способ уронить накат на чистой базе (§9, Р-5).
