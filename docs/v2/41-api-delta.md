...
> [уточнено, реализация PR-34] `GET /people/:id/activity` скоупа на входе не требует: свою ленту человек видит без
> него, чужую — `person.activity.view_others` в области текущей точки человека или на весь тенант, проверяющий —
> ленту людей из своей очереди проверки за 90 дней по данным (`38` §2, §7.11). Прежний контракт этого пути базового
> ТЗ (журнал действий из `audit_log`, `people.view`) переехал на `GET /people/:id/action-log` — это путь основного
> ТЗ (`docs/04` §4.11), а не пакета: в счёт 23 он не входит.
...
| POST | `/j/:token/apply/:aid/resume` | multipart | `200 {asset_id}` | `413 media.too_big`, `415 file.type` |
...
| `limit.users_exceeded` | 409 | найм упирается в лимит `users_active` — **отменён** `44` В-16: `409 limit_exceeded` с `details.axis=users_active` (fix-seat-limit) | «Вичерпано ліміт співробітників за тарифом» |
...
| `media.too_big` | 413 | резюме больше допустимого | «Файл завеликий» |
...
| `media.too_big` | 413 | аудиоответ больше 25 МБ | «Файл завеликий» |
...
| `media.too_big` | 413 | файл больше допустимого — коллизия написания решена, §8.2.4 | «Файл завеликий» |
...
| `rating_only_filter_forbidden` | 422 | індекс залученості — единственное условие массового «Архівувати» / «Заблокувати» (`38` §7.3, §13 к. 3) | «Індекс залученості не може бути єдиною умовою масового архівування чи блокування. Додайте ще одну умову (точка, посада, мітка…) або позначте людей вручну» |

> [дополнено, реализация PR-35] Код `rating_only_filter_forbidden` строкой выше в реестре не было,
> хотя его требует критерий `38` §13 к. 3 — добавлен. `rating_forbidden` не заводился: «нет скоупа»
> — одно написание `forbidden` (`44` В-16 §8.2.7). `recalc_too_often` — как в таблице; сверх неё у
> пересчёта уволенного — `409 person_archived` (значение зафиксировано, `38` §7.2).

> [исправлено, реализация PR-33] Коллизия §8.2.6 закрыта написанием `38` §10:
> `absence_record.range_invalid` (у отсутствия проверяющего — `reviewer_absence.range_invalid`,
> PR-19). Сверх таблицы: `409 absence_status_invalid` (статус записи назад), `409 absence_cancelled`
> (правка отменённой), `409 person_archived`. Чужие отсутствия видит тот же `person.absence.manage`
> в области точки; свои — человек без скоупа (`38` §2). Ранее: «`absence_range_invalid` | 422».
...
> [исправлено фазой 1, `43` §5 и §3.4, `44` В-16] Счётчики выше — снимок пакета до сверки,
> оставлен как есть. Часть перечисленных коллизий фазой 1 уже решена: **`file.too_large`**
> (обе записи документов пакета) заменяется на действующее в коде написание **`media.too_big`**
> (§8.2.4); **`validation_failed`** в `UPPER_SNAKE` (документ `36`) приводится к нижнему регистру
> вместо второго кода (§8.2.5); пять вариантов «нет скоупа» (`forbidden`, `forbidden`,
> `forbidden`, `scope`, `forbidden`) сводятся к одному действующему в `server/api` написанию
> **`forbidden`** (§8.2.7). Остальные коллизии из перечня решаются вне PR-02, в PR своего
> документа-владельца (`44` В-16, В-9).
...
**8.2.4. `file.too_large` против `file.too_large`. Решено фазой 1 (`43` §5, `44`).** `29` и `30`
писали через точку, `34` — через подчёркивание; ни одно из написаний не используется кодом:
`server/services/media.ts` отдаёт `too_big` / `resource_too_big`, наружу уходит `media.too_big`
(`media/upload-url.post.ts`, `docs/04-api.md` §4.15). Принято действующее написание —
**`media.too_big`** — и оба документа пакета (`29`, `30`, а также `34`) приведены к нему.

**8.2.5. `validation_failed` против `validation_failed`. Решено фазой 1.** `36` §10 вводил
`400 validation_failed` в `UPPER_SNAKE` при том, что базовое ТЗ уже имеет `400 validation_failed`
в нижнем регистре с `details.issues`. Это не новый код, это второе написание существующего.
Девять кодов документа `36`, записанных в `UPPER_SNAKE`, приведены к нижнему регистру.
...
**8.2.7. Пять написаний «нет скоупа». Решено фазой 1 (`43` §5, `44`).** Базовое ТЗ: `403
forbidden`. `32`: `403 forbidden`. `37`: `403 forbidden`. `30`: `403 scope` и
`403 interview.listen`, `403 ai.audit`. `38`: `403 forbidden`, `403 note_forbidden`,
`403 activity_forbidden`, `403 rating_forbidden`. `31`: `403 library.forbidden`,
`403 library.not_author`, `403 library.manage_required`. В `server/api` действует одно
написание — **`forbidden`**, 25 файлов. Принято: оставить только `forbidden`; пять уточняющих
вариантов из `30`, `31`, `38` вычеркнуты из документов пакета (различие в тексте объяснения
передаётся телом ответа, а не отдельным кодом).
...
1. **Ограничения частоты для закрытого контура.** Ось `api_rate_rpm` (`35` §7.1) даёт `429` с
   ...
   **Решено `44` В-20:** окно — на токен, фиксированное 60 с, по умолчанию 60 запросов
   (`DEFAULT_API_PER_MINUTE`), ось — существующая колонка `tenant_limits.api_per_minute`.
2. **Bearer-токен тенанта и новые эндпоинты.** `04-api.md` §4.1 допускает
   ...
   **Решено `44` В-20, реализовано PR-39:** чёрного списка путей нет — флаг `sessionOnly` на
   скоупе (`shared/domain/roles.ts` `SCOPE_FLAGS`): такой скоуп токену не выдаётся
   (`422 scope_not_tokenable`), у старого токена вычёркивается (`403 forbidden`). `/platform/*`
   по Bearer недоступен архитектурно (cookie `lola_ops` разбирается раньше ветки Bearer);
   заметки о человеке и прослушивание собеседования закрыты флагом на своих скоупах.
3. **Вебхуки наружу.** ~~`04-api.md` §4.18 перечисляет восемь событий. Пакет вводит события,
   ...
   **Решено `44` В-18, реализовано PR-37**: `WEBHOOK_EVENTS` = 10 значений, три новых события
   добавлены с PII-free payload, `04-api.md` §4.18 и `docs/09-integrations.md` §9.5 приведены
   к фактическому составу.
