...
| Таблица | Вердикт | Примечание |
|---|---|---|
| `review_delegations` · `review_routing_rules` · `reviewer_capacity` · `reviewer_absences` · `review_sla_events` · `reviewer_stats_daily` · `content_time_norms` | нет (7) | Очередь проверки сегодня — два независимых запроса: `server/services/attempts.ts:653`, `:746` поверх `attempt_answers` и `server/services/workshops.ts:230` поверх `workshop_submissions`, эндпоинты `/review/answers`, `/review/submissions`, `/review/workshops`. `workshop_submissions` уже несёт `reviewer_id`, `claimed_at`, `sla_due_at` — половина состояния одного таба |
...
| **П-13** очередь проверки | обязательный + блокирующее замечание | **не применён** | `docs/13-workshops.md` не правлен. Фактически: очередь — **два запроса**, а не объект БД (`server/services/attempts.ts:653`, `server/services/workshops.ts:230`; эндпоинты `/review/answers`, `/review/submissions`, `/review/workshops`). Ни таблицы, ни представления, ни matview. Блокирующее замечание (повышение до таблицы) — **открыто**, см. Р-3 |
...
| **П-16.3** колонка рейтинга | желательный | **не применён** → [решено, PR-35] применён: `users.rating_pct`, колонка «%», запрет единственного условия, галка выгрузки | `users.rating_pct` нет |
...
> [исправлено фазой 2, пересчёт по таблице вердиктов §2.3] Ранее: «29 проверяемых пунктов»
> и «не применён — 19». В таблице §2.3 **32 строки**: номера П-17 и П-25 в ней тоже разбиты
> на подпункты (П-17.1, П-17.2, П-25.1, П-25.2, П-25.3), а §2.1 учитывал разбивку только
> у П-12, П-16 и П-24. При десяти частично применённых не применённых получается **22**,
> а всего пунктов — **32**. На вердикты это не влияет, влияет на покрытие: план фазы 2
> (`45-plan.md` §8.1) раскладывает по PR все 32.

| Вердикт | Пунктов (из 32) |
|---|---:|
...
| **8.7.3** Вебхуки наружу: события пакета не добавлены | **закрыт (PR-37)** | `WEBHOOK_EVENTS` (`server/services/webhooks.ts`) — 10 значений: семь прежних плюс `candidate.hired`, `vacancy.application_received`, `offboarding.completed`. Все три шлются из реального кода: `hireCandidate()`, `completeOffboarding()`, `convertApplication()` (последний появился в `main` тем же днём, PR-16 #109, и PR-37 подключил вызов сразу после ребейза). Payload всех трёх — только идентификаторы и время, проверено тестом на реальных данных |
...
2. **`review_queue_items`: витрина → таблица.** Обострился (Р-3): объекта нет вовсе, очередь
   ...
   `server/services/attempts.ts` и `server/services/workshops.ts`; или оставить запрос и перенести 21 колонку состояния в
...
> [исправлено фазой 1, проверка по коду] Ранее в §1.4, §2.3 (П-13), §4.1 (Р-3) и §6 п. 2:
> «очередь собирается запросом в `server/services/queue.ts`». Фактически `queue.ts` — обёртка
> над pg-boss (очереди задач и расписания cron), а очередей проверки **две, независимые**:
> `listReviewAnswers` / `reviewQueue` в `server/services/attempts.ts:653` и `:746` и
> `reviewQueue` в `server/services/workshops.ts:230`. На вердикт Р-3 это не влияет, но меняет
> цену вопроса `В-2` в `44-decisions.md`: переписывать придётся два сервиса, а не один.
