...
| **8.7.3** Вебхуки наружу: события пакета не добавлены | **закрыт (PR-37)** | `WEBHOOK_EVENTS` (`server/services/webhooks.ts`) — 10 значений: семь прежних плюс `candidate.hired`, `vacancy.application_received`, `offboarding.completed`. `candidate.hired`/`offboarding.completed` шлются из `hireCandidate()`/`completeOffboarding()`; `vacancy.application_received` зарегистрирован и покрыт тестом контракта payload, реальный вызов подключит PR-16 (приёма отклика в `main` ещё нет). Payload всех трёх — только идентификаторы и время |
