...
export default defineEventHandler(async (event) => {
  ...
  const r = await decideRequest({ tenantId: a.tenantId, actorId: a.userId }, kind, getRouterParam(event, 'id')!, p.data.decision, { comment: p.data.comment, isHr: can(a, 'development.manage'), isAdmin: can(a, 'settings.tenant') })
  if (!r.ok) return apiError(event, r.code === 'not_found' ? 404 : r.code === 'self' ? 403 : 409, `request.${r.code}`, r.code === 'self' ? 'Власну заявку не погоджують' : r.code === 'bad_step' ? 'Не ваш етап погодження' : r.code === 'admin_required' ? 'Сума вище порогу — потрібне рішення адміністратора' : 'Заявку не знайдено')
  return apiData(r)
})
