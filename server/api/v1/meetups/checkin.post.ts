...
import { checkin } from '../../../services/meetups'
import { apiData, apiError } from '../../../utils/apiResponse'
export default defineEventHandler(async (event) => {
  const a = await requireScope(event, 'meetup.enroll')
  const p = z.object({ token: z.string().min(10).max(200) }).safeParse(await readBody(event))
  ...
  if (!r.ok) {
    ...
    return apiError(event, st, r.code === 'expired' ? 'qr.expired' : r.code, msg, { meetupId: r.meetupId, seatsLeft: r.seatsLeft })
  }
  return apiData(r)
})
