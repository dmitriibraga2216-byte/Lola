...
const schema = z.object({ title: z.string().min(3).max(200).optional(), body: bodySchema.optional(), isPinned: z.boolean().optional(), requiresAck: z.boolean().optional(), status: z.enum(['draft', 'published', 'archived']).optional(), coverKey: z.string().nullable().optional(), lead: z.string().max(300).nullable().optional(), publishAt: z.string().datetime({ offset: true }).nullable().optional(), unpublishAt: z.string().datetime({ offset: true }).nullable().optional(), commentsEnabled: z.boolean().optional(), categoryId: z.string().uuid().nullable().optional() })
