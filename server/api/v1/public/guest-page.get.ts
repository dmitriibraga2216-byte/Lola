 ...
 * Тенант — по поддомену Host (docs/25 §16.1), в dev — `?slug=`. Неизвестный — 404 без подробностей.
 */
export default defineEventHandler(async (event) => {
  const q = z.object({ slug: z.string().max(40).optional() }).safeParse(getQuery(event))
  const slug = slugFromHost(getHeader(event, 'host')) ?? (q.success ? q.data.slug : undefined)
