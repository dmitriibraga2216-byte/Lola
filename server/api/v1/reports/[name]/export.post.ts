...
export default defineEventHandler(async (event) => {
  ...
  const e = await requestExport({ tenantId: a.tenantId, actorId: a.userId, activeRoleId: a.activeRole?.id ?? null }, { report: getRouterParam(event, 'name')!, filters: p.success ? p.data.filters : {}, format: p.success ? p.data.format : 'xlsx' })
