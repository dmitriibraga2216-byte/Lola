...
export default defineEventHandler(async (event) => {
  ...
  if (q.kind === 'turnover') {
    ...
    return apiData({ kind: 'turnover', from, to, rows: await turnoverReport(ctx, from, to) })
  }
  if (q.kind === 'inactive') return apiData({ kind: 'inactive', days: q.days, rows: await inactiveReport(ctx, q.days) })
  const asOf = q.asOf ?? new Date().toISOString().slice(0, 10)
  return apiData({ kind: 'staffing', asOf, rows: await staffingReport(ctx, asOf) })
})
