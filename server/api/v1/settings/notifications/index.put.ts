...
export default defineEventHandler(async (event) => {
  ...
  const row = await withTenant(a.tenantId, a.userId, async (tx) => {
    ...
    const textChanged = !before || before.body !== d.body || (before.subject ?? null) !== (d.subject ?? null)
    const version = before ? (textChanged ? before.version + 1 : before.version) : 1
    const set = { subject: d.subject ?? null, body: d.body, isEnabled: d.isEnabled, buttons: d.buttons ?? [], isMandatory: d.isMandatory ?? false, throttle: d.throttle ?? null, escalateAfterHours: d.escalateAfterHours ?? null, ignoreQuietHours: d.ignoreQuietHours ?? false, version, updatedAt: new Date() }
    ...
    if (textChanged) await tx.insert(notificationTemplateVersions).values({ tenantId: a.tenantId, templateId: r!.id, version, subject: d.subject ?? null, body: d.body, authorId: a.userId })
