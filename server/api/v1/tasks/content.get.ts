...
export default defineEventHandler(async (event) => {
  ...
  if (!p.success) return apiError(event, 400, 'validation_failed', 'Невідомий тип контенту', { issues: p.error.issues })
  return apiData(await listContent({ tenantId: a.tenantId, actorId: a.userId }, p.data.type, p.data.q))
})
