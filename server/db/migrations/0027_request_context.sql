...
-- Единый технический контекст журналов (CLAUDE.md п. 14): старые ip/user_agent переносятся в request_context
UPDATE "audit_log" SET "request_context" = jsonb_strip_nulls(jsonb_build_object('ip', host("ip"), 'userAgent', "user_agent")) WHERE "request_context" IS NULL AND ("ip" IS NOT NULL OR "user_agent" IS NOT NULL);--> statement-breakpoint
UPDATE "security_log" SET "request_context" = jsonb_strip_nulls(jsonb_build_object('ip', host("ip"), 'userAgent', "user_agent")) WHERE "request_context" IS NULL AND ("ip" IS NOT NULL OR "user_agent" IS NOT NULL);--> statement-breakpoint
UPDATE "sessions" SET "request_context" = jsonb_strip_nulls(jsonb_build_object('ip', host("ip"), 'userAgent', "user_agent")) WHERE "request_context" IS NULL AND ("ip" IS NOT NULL OR "user_agent" IS NOT NULL);
