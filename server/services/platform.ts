 ...
 * блокировка человека сразу освобождает место. Операции, которые **занимают** место (создание,
 * приглашение, импорт, разблокировка, восстановление, найм, первый вход), зовут не эту функцию, а
 * `assertSeatsWithinLimit()` из `tenantLimits.ts` — в своей транзакции и под блокировкой мест:
 * число отсюда, посчитанное отдельным подключением, между проверкой и записью устаревает
 * (fix-seat-limit). Здесь остаются отчёт и пополнение счётчика.
 *
