...
describe('этап 9: комплексные тесты (docs/18 §13.6)', () => {
  async function makeQuiz(title: string, passScore: number) {
    const q1 = (await createQuestion(ctx(), { bankId, kind: 'single', stem: [{ id: 'b', type: 'text', html: `<p>${title} 1?</p>` }], options: [{ id: 'a', text: 'A' }, { id: 'b', text: 'B' }], answer: { correctId: 'a' }, isCritical: false, difficulty: 1, points: 1, partialCredit: true, negativeMarking: false, tags: [] })).id
