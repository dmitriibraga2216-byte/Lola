...
describe('docs/31 рядок PersonCard: «Рейтинг» у зведенні «Навчання» для чужої людини', () => {
  it('personLearning рахує currentRating так само, як /me/study-history (studyHistory) — бали з книги (D-069)', async () => {
    ...
    // Два виконані завдання з балами — два рядки книги з наростаючим залишком
    await admin`insert into points_ledger (tenant_id, user_id, currency, delta, balance_after, event, ref_id, title) values
      (${tenantId}, ${uid}, 'points', 50, 50, 'task_completed', gen_random_uuid(), 'Курс'),
      (${tenantId}, ${uid}, 'points', 30, 80, 'task_completed', gen_random_uuid(), 'Тест')`
    try {
      ...
      expect(learning.currentRating).toBe(80) // курс + тест, у балах, а не штуках
    }
    finally {
      await admin`delete from points_ledger where user_id = ${uid}`
