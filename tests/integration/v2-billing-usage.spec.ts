...
describe('35 §7.9, §13 к. 8: предупреждения 80 % и 100 %', () => {
  ...
  it('[fix-night-debts §3] нулевой лимит: ось, которую не трогали, — ok; ось с попыткой — exceeded, не ok', () => {
    // Раньше `limit <= 0` безусловно давало 'ok' — баннер об исчерпанной оси с нулевым лимитом
    // (тариф вовсе без ИИ) не поднимался никогда, даже когда вызов реально отклонялся.
    expect(levelOf(0, 0)).toBe('ok') // никто не пользовался — баннеру мигать нечем
    expect(levelOf(1, 0)).toBe('exceeded') // попытка была (вызывающий передаёт used с её учётом)
    expect(levelOf(5, 0)).toBe('exceeded')
    expect(levelOf(0, -1)).toBe('ok') // отрицательный лимит в проде не встречается, но не должен падать
  })
