/**
 * Единый список таблиц пакета `docs/v2` — по факту миграций `NNNN_v2_*.sql` после PR-40.
 *
 * > [исправлено, PR-40: все PR плана `docs/v2/45-plan.md` в `main`, список полон и зафиксирован на
 * > длину] Ранее: «Здесь список **не копируется целиком**: ни одна из этих таблиц ещё не создана
 * > миграцией, и жёстко зашитый полный перечень сделал бы контрактные тесты красными уже сегодня».
 *
 * Состав — **74 тенантные и 3 платформенные**: 71 тенантная `docs/v2/40-data-model-delta.md` §6.1
 * минус три, которые решениями не заводятся (`org_structure_conflicts`, `person_notes`,
 * `storage_quota_addons`), плюс шесть сверх (`review_queue_items`, `user_notes` и четыре таблицы
 * патчей PR-39); платформенные — две `40` §6.2 и `platform_announcements`. Каждое расхождение —
 * строкой таблицы «было в пакете → стало → каким PR и почему» в `docs/v2/40` §2.13.
 *
 * Длину и состав держит контрактный тест №6 (`v2-contract-06-package-tables.spec.ts`): он сверяет
 * списки с `40` §6.1 поимённо и с `create table` миграций пакета — таблица пакета, созданная миграцией
 * и не внесённая сюда, валит его. Правило для новой таблицы пакета прежнее (`45` PR-01): внести имя
 * сюда, строку — в `40` §2.13, число — в константу теста №6; тогда контракты 1–3 и 5 (RLS, индекс,
 * FK, запрет правил прохождения) становятся для неё содержательными без отдельной правки.
 */

/** Тенантные таблицы пакета (docs/v2/40 §6.1 ± §2.13) — 74, длину держит контрактный тест №6. */
export const V2_PACKAGE_TENANT_TABLES: string[] = [
  ...
  // ключ `users_vacancy_id_fk` проверяет девятый контрактный тест `v2-contract-09-deferred-fk.spec.ts`
  ...
  // 29. Публикация и генерация текста (PR-17, миграция 0086_v2_vacancy_publish).
  ...
  // 38. Заметки и документы человека (PR-32, миграция 0082_v2_person_notes_docs). `user_notes` —
  ...
  // П-24.1 (второй фактор входа). Сверка PR-40 «на длину» их прибавила (`docs/v2/40` §2.13,
  // контрактный тест №6). `absence_norms` — из 71 (`38` §3.6): блок норм
  ...
  'absence_norms',
  // 38. Факты отсутствий (PR-33, миграция 0090_v2_absences): по ним считается остаток нормы и
  ...
  'content_time_norms',
  // 30. Провайдер модели и учёт вызовов (PR-27, миграция 0091_v2_ai_providers). Профили
  // платформы копируются тенанту строками (`30` §3.2 [решение]) — поэтому и профиль тенантный,
  // под RLS, а не платформенный справочник с `tenant_id is null`.
  'ai_providers',
  'ai_calls',
  // 38. Лента и карта активности (PR-34, миграция 0093_v2_user_activity): события со снимком пояса и
  ...
  'user_activity_daily',
  // 38. Индекс навчальної залученості (PR-35, миграция 0094_v2_person_rating): снимок расчёта с числами
...
/** Платформенные таблицы пакета, вне RLS (docs/v2/40 §6.2 + `platform_announcements`, §2.13) — 3. */
